...
#include "country.h"
#include  <stdbool.h>
#include "DALIstring.h"
#include "string_dimmer_slop_page.h"
//------------------------------------------------------------------------------
//         Definitions
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
typedef struct {
 unsigned char win_value;
 unsigned char corid_value;
 unsigned char board_value;
 unsigned char win_blink;
 unsigned char corid_blink;
 unsigned char board_blink;
} PAGE_VIEW_ITEM;     //for display value
...
static unsigned char pageScroll1 = 0;    // 2.4.0
...
PAGE_HANDLER pageHandler[] = {
    ...
    DALINoGroupHandler,              //PAGE_INDEX_NOGROUP
    DimmerSlopePageHandler,          //PAGE_INDEX_DIMSLOPE
    AuxiliaryInputPageHandler,       //PAGE_INDEX_AUXINPUT
    OnLevelPageHandler,              //PAGE_INDEX_ONLEVEL
    HourPageHandled,                 //page_index_hour
    ExtendModePageHandler,           //PAGE_INDEX_EXTENDMODE
    ExtendModeStatePageHandler,      //PAGE_INDEX_EXTENDMODESATE
    ExtendModeFunctionPageHandler,   //PAGE_INDEX_EXTENDFUNC
    ExtendStandbyPageHandler,        //PAGE_INDEX_EXTENDSTANDBY
    ExtendStandbyDelayHandler,       //PAGE_INDEX_EXTENDDELAY
    ExtendOnLevelPageHandler,        //PAGE_INDEX_EXTENDONLEVEL
    OperationTimeOnPageHandler,       //PAGE_INDEX_OPTIME
    ExtendSchedulePageHandler,        //PAGE_INDEX_EXTENDSCHEDULED 
#ifdef ROOM_CONTROL
...
unsigned char pageCurrent = PAGE_INDEX_MAIN;//PAGE_CREATE_ZONE;//PAGE_INDEX_MAIN;//PAGE_INDEX_DALI_SLAVE;//PAGE_INDEX_MAIN;//PAGE_CREATE_ZONE;// PAGE_INDEX_MAIN;//PAGE_VIEW_AREA;//PAGE_CHOOSE_ZONE;//PAGE_INDEX_MAIN;
...
extern const unsigned char *DALI_sensor_type_name[];
extern const unsigned char *DALI_ballast_name[];
extern const unsigned char *DALI_sensor_name[];
extern const unsigned char *DALI_changeToSlave_name[];
extern const unsigned char *DALI_eraseDaliSys_name[];
extern const unsigned char *DALI_warning_name[];
extern const unsigned char *DALI_warning_long_name[];
extern const unsigned char *DALI_warning_long_name[];
extern const unsigned char *DALI_erase_long_name[];
...
void _param_pageNOback_funckey_initializer(unsigned char availability)
{
    memset(pageFunckeyItems, 0, sizeof(pageFunckeyItems));
    if (availability & FUNCKEY_READ_MASK)
    {
...
#define PAGE_SUBREADPARAMS_BODY_Y_START     (PAGE_SUBREADPARAMS_RECT_Y_START+PAGE_SUBREADPARAMS_BODY_INDENT)
    ...
    const unsigned char *stringDALI ={"ERROR 5 Invalid Parameter Need two zones"};
            ...
            if (PARAMS_IsParameterAvailable(&sensorParams, currentParameter))
            {
                ...
                while (retry < PAGE_PARAM_READ_RETRY)
                {
                    if (result = IR_ReadSensorParameter(&sensorParams, currentParameter))
                    {
                        ...
                        case 1:
                            //string = NULL;
                            if ((currentParameter == PARAM_INDEX_STANDBYLEVEL )|| (currentParameter == PARAM_INDEX_ON_LEVEL ) || (currentParameter == PARAM_INDEX_EXT_ON_LEVEL)|| (currentParameter == PARAM_INDEX_EXT_STANDBY_LEVEL))
                            {
                                PARAMS_InterpretStandbyLevel(&sensorParams,currentParameter);
                            }
...
unsigned char const* find_week_num(unsigned char WeekDayNum,LANGUAGES lang)
{
  return (strings_date_week_title[WeekDayNum-1+lang*7]);
}
#pragma optimize=none
void _list_parameters(SENSOR_PARAMS const *pSensorParams, unsigned char DisplayReadOnlyItems, unsigned char Swap)
{
#define PARAMETERS_AMOUNT_MAX           57      // 2.4.0, was 29
...
#define PARAMETERS_LINE_4               (PARAMETERS_LINE_3 + PARAMETERS_LINE_HEIGHT + PARAMETERS_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        pReadParameters = (PAGE_READ_PARAMS *)malloc(PARAMETERS_AMOUNT_MAX*sizeof(PAGE_READ_PARAMS));
        ...
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_DAYLIGHTSETPOINT))
        {
            if (PARAMS_IsLightController(pSensorParams))    // 2.4.0
            {
                (pParametersTemp)->pName = strings[STRINGS_INDEX_SETPOINT+language];
            }
            else
            {
                (pParametersTemp)->pName = strings[STRINGS_INDEX_DAYLIGHTSETPOINT+language];
            }
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            pValuesTemp += dectoascii(pValuesTemp, 5*PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_DAYLIGHTSETPOINT), 0);
            if (PARAMS_SensorIsHFOnly(&sensorParams) && !is_DALI_sensor(&sensorParams))     // 2.2.0
            {
                pValuesTemp++;
            }
            else
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_MODE+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_MODE+STRINGS_TOTAL_LANGUAGES*(unsigned char)PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_MODE)+language]);
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
        }
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_DETECTIONSCHEME+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
...
//            strcpy((char *)pValuesTemp, " > ");
//            pValuesTemp += 3;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_DISABLED+STRINGS_TOTAL_LANGUAGES*(unsigned char)PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_DETECTIONSCHEME)+language]);
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
        }
        ...
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_INITIAL))
        {
            ...
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_INITIAL+language]);
            pValuesTemp += strlen((char const *)pValuesTemp);
            strcpy((char *)pValuesTemp, " > ");
            pValuesTemp += 3;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_DISABLED+STRINGS_TOTAL_LANGUAGES*(unsigned char)PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_INITIAL)+language]);
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
        }
        ...
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_MAINTAIN))
        {
            ...
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_MAINTAIN+language]);
            pValuesTemp += strlen((char const *)pValuesTemp);
            strcpy((char *)pValuesTemp, " > ");
            pValuesTemp += 3;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_DISABLED+STRINGS_TOTAL_LANGUAGES*(unsigned char)PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_MAINTAIN)+language]);
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
        }
        ...
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_RETRIGGER))
        {
            ...
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_RETRIGGER+language]);
            pValuesTemp += strlen((char const *)pValuesTemp);
            strcpy((char *)pValuesTemp, " > ");
            pValuesTemp += 3;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_DISABLED+STRINGS_TOTAL_LANGUAGES*(unsigned char)PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_RETRIGGER)+language]);
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
        }
        ...
        {
          (pParametersTemp)->pName = strings[STRINGS_INDEX_ALERT+language];
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
          if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_ALERT) == (unsigned short)0x2)  //0x2 refers to audible alert
          {
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_AUDIBLE+language]);
          }
          else  if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_ALERT) == (unsigned short)0x1)  //0x2 refers to audible alert
          {
            strcpy((char *)pValuesTemp, "Visual");
          }
          else
          {
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_DISABLED+language]);
          }
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
        }
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_STANDBYLEVEL+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_STANDBYDELAY+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_STANDBYDELAY) == 0)
            {
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_NOSTANDBY+language]);
                pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
            }
            else if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_STANDBYDELAY) == 255)
            {
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_UNLIMITED+language]);
                pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
            }
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_SENDCONDITION+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_CYCLICAL+STRINGS_TOTAL_LANGUAGES*(unsigned char)PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_SENDCONDITION)+language]);
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
        }
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_DEADBAND+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_TIMEBASE+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_BANDWIDTH+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_SUMMERHOUR+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_SUMMERHOUR) <= 2)
            {
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_NOCHOICE+STRINGS_TOTAL_LANGUAGES*(unsigned char)PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_SUMMERHOUR)+language]);
            }
            else
            {
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_NONE+language]);
            }
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_STARTDAY+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_ENDDAY+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_COUNTRY+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            strcpy((char *)pValuesTemp, (char const *)valuesCountry[PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_COUNTRY)*5+language]);
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
        }
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_LONGITUDE+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_LATITUDE+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_LIGHTREGULATION+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_LIGHTREGULATION))
            {
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_ENABLED+language]);
            }
            else
            {
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_DISABLED+language]);
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_PROVISIONOFLIGHT+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            ...
            {
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_AUTO+language]);      // 2.4.0
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_LOOPTYPE+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_CLOSELOOP+STRINGS_TOTAL_LANGUAGES*(unsigned char)PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_LOOPTYPE)+language]);
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
        }

///////////type position
        //dimmer slope r
        if(PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_DIM_R))
        {
          (pParametersTemp)->pName = strings_dim_rise_titile[language];
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
          
          pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_DIM_R), 2);
          strcpy((char *)pValuesTemp, strings_dim_sec_titile[language]);
          pValuesTemp += (sizeof(strings_dim_sec_titile[language])+1);
        
        }
        
                //dimmer slope f
        if(PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_DIM_F))
        {
          (pParametersTemp)->pName = strings_dim_fall_titile[language];
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
          
          pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_DIM_F), 2);
          strcpy((char *)pValuesTemp, strings_dim_sec_titile[language]);
          pValuesTemp += (sizeof(strings_dim_sec_titile[language])+1);
        ...
        //aux mode
        if(PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_AUX_MODE))
        {
          (pParametersTemp)->pName = strings_aux_input_mode_titile[language];
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
           strcpy((char *)pValuesTemp, (char const *)((PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_AUX_MODE)==1)?strings_button_titile[language]:strings_switch_titile[language]));

          //pValuesTemp += &strings_button_titile[language];//((PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_AUX_MODE)==1)?strings_button_titile[language]:strings_switch_titile[language]);
          //strcpy((char *)pValuesTemp, strings_dim_sec_titile[language]);
          //pValuesTemp += (sizeof(strings_dim_sec_titile[language])+1);
                     pValuesTemp += (strlen((char const *)pValuesTemp) + 10); 
        }
        //aux pul time
       if(PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_AUX_PULE))
        {
          (pParametersTemp)->pName = strings_aux_input_short_pulse_titile[language];
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
          
          pValuesTemp +=  dectoascii(pValuesTemp, (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_AUX_PULE))/10, 1);
          strcpy((char *)pValuesTemp, ".");
          pValuesTemp += 1;
          pValuesTemp += dectoascii(pValuesTemp, (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_AUX_PULE))%10, 1);
          
          strcpy((char *)pValuesTemp, strings_dim_sec_titile[language]);
          pValuesTemp += (sizeof(strings_dim_sec_titile[language])+3);
          
          
        }
        
        // on level
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_ON_LEVEL))
        {
            (pParametersTemp)->pName = strings_on_level_titile[language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_ON_LEVEL), 0);
            *pValuesTemp++ = '%';
            *pValuesTemp++ = '\0';
        }
        
                //ext state
        if(PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_MODE_STATE))
        {
          (pParametersTemp)->pName = extended_mode_state[language];//"extended mode state";
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
          strcpy((char *)pValuesTemp, (char const *)((PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_MODE_STATE)==1)?strings_extend_mode_state_titile[language+LANGUAGES_TOTAL]:strings_extend_mode_state_titile[language]));
          pValuesTemp += (strlen((char const *)pValuesTemp) + 1); 
        }
        
                // ext Mode
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_FUN_MODE))
        {
            (pParametersTemp)->pName = extended_mode_function[language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_MODE+STRINGS_TOTAL_LANGUAGES*(unsigned char)PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_MODE)+language]);
            pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
                ...
                // ext Standby level
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL))
        {
            (pParametersTemp)->pName =extended_mode_sl[language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL), 0);
            *pValuesTemp++ = '%';
            *pValuesTemp++ = '\0';
        }

        // ext Standby delay
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_STANDBY_DELAY))
        {
            (pParametersTemp)->pName = extended_mode_sd[language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_STANDBY_DELAY) == 0)
            {
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_NOSTANDBY+language]);
                pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
            }
            else if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_STANDBY_DELAY) == 255)
            {
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_UNLIMITED+language]);
                pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
            }
            else if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_STANDBY_DELAY) <= 60)
            {
                pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_STANDBY_DELAY), 0);
                strcpy((char *)pValuesTemp, " Sec");
                pValuesTemp += 5;
            }
            else if (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_STANDBY_DELAY) <= 120)
            {
                pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_STANDBY_DELAY)-60, 0);
                strcpy((char *)pValuesTemp, " Min");
                pValuesTemp += 5;
            }
        }
                ...
                //ext on level
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_ON_LEVEL))
        {
            (pParametersTemp)->pName = extended_mode_ol[language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_ON_LEVEL), 0);
            *pValuesTemp++ = '%';
            *pValuesTemp++ = '\0';
        }
        //week day
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_WEEK_DAY))
        {
          (pParametersTemp)->pName = strings_week_day_title[language];
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
          strcpy( (char *)pValuesTemp,(find_week_num(PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_WEEK_DAY),language)));
          pValuesTemp += (strlen( (const char*)pValuesTemp)+1);         
          
        }

        
         //extended scheduled day
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_SCHEDULD_DAY))
        {
          (pParametersTemp)->pName = string_scheduled_day_titile[language];
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
          char i=0;
          
          strcpy( (char *)pValuesTemp,  (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_SCHEDULD_DAY)>>i &0x01)==1?strings_scheduled_week[0+i+7*language] : " ?" );
          for(i=1;i<7;i++) 
            strcat( (char *)pValuesTemp,  (PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_SCHEDULD_DAY)>>i &0x01)==1?strings_scheduled_week[0+i+7*language] : " ?" );
          pValuesTemp += (strlen( (const char*)pValuesTemp)+1);         
          
        }
        
        //extended scheduled start hour
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_START_HOUR) && PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_START_MIN))
        {
          (pParametersTemp)->pName = "Extended mode start hour";
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
          
          dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_START_HOUR), 2);
          strcat( (char *)pValuesTemp, "h" );
          pValuesTemp+=3;
          dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_START_MIN), 2); 
          pValuesTemp += (1+1+1);         
           *pValuesTemp++ = '\0';
           
        }
        
          //extended scheduled stop hour      
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_STOP_HOUR) && PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_STOP_MIN))
        {
          (pParametersTemp)->pName = "Extended mode stop hour";
          (pParametersTemp)->pValue = pValuesTemp;
          pParametersTemp->ValueType = 0; // string
          pParametersTemp++;
          
          dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_STOP_HOUR), 2);
          strcat( (char *)pValuesTemp, "h" );
          pValuesTemp+=3;
          dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_STOP_MIN), 2); 
          pValuesTemp += (1+1+1);         
           *pValuesTemp++ = '\0';
        }
                // extended hour
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_REAL_HOUR) || PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_REAL_MIN) || PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_REAL_SEC))
        {
            if (PARAMS_IsLightController(pSensorParams))    // 2.4.0
            {
                pParametersTemp->pName = strings_operating_time_hours_titile[language];//"extended hour";
            }
            else
            {
                pParametersTemp->pName = strings_operating_time_hours_titile[language];
            }
            ...
            else
            {
                pParametersTemp->pValue = pValuesTemp;
                pParametersTemp->ValueType = 0; // string
                if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_REAL_HOUR))
                {
                    pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_REAL_HOUR), 0); //hour value
                    if (language == German)     // 2.4.0
                    {
                        strcpy((char *)pValuesTemp, " Std ");
                        pValuesTemp += 5; //strlen((char const *)" Std ");
                ...
                if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_REAL_MIN))
                {
                    pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_REAL_MIN), 0); //minute value
                    strcpy((char *)pValuesTemp, " Min ");
                    pValuesTemp += 5; //strlen((char const *)" Min ");
                }
                if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_EXT_REAL_SEC))
                {
                    pValuesTemp += dectoascii(pValuesTemp, PARAMS_GetParameterValue(pSensorParams, PARAM_INDEX_EXT_REAL_SEC), 0); //second value
                    if (language == German)     // 2.4.0
                    {
                        strcpy((char *)pValuesTemp, " Sek");
                    }
...
//            (pParametersTemp)->pName = "DeviceID Zigbee";   // 2.4.0
            (pParametersTemp)->pName = strings[STRINGS_INDEX_DEVICE_ID+language];
...
*/
    ...
    //dd    // " Master / Slave " 
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_TYPE+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
            if( is_KNX_sensor( &sensorParams ) )
            {
                 strcpy((char *)pValuesTemp, (char const *)KNX_sensor_type_name[(sensorParams.Type.Value & 0x000F)*LANGUAGES_TOTAL+language]);
                 pValuesTemp += (strlen((char const *)pValuesTemp) + 1);                
            }
            
            if(is_PRECIR_sensor( &sensorParams ))
            {
              switch (sensorParams.Type.Value)
              {
              case PARAM_TYPE_HF_PRECIR : strcpy((char*)pValuesTemp,"PRECIR HF");
              pValuesTemp += (strlen((char const *)pValuesTemp) + 1);  
              break;
              case PARAM_TYPE_HF_PRECIR_CLOCK:strcpy((char*)pValuesTemp,"PRECIR HF with clock");
              pValuesTemp += (strlen((char const *)pValuesTemp) + 1);  
              break;
              case PARAM_TYPE_HF_PRECIR_BILEVEL:strcpy((char*)pValuesTemp,"PRECIR bilevel");
              pValuesTemp += (strlen((char const *)pValuesTemp) + 1);  
              break;
              case PARAM_TYPE_HF_PRECIR_ESSENTIAL:
                strcpy((char*)pValuesTemp,"PRECIR HF essential");
                pValuesTemp += (strlen((char const *)pValuesTemp) + 1);  
                break;
              }
            } 
            ...
            {
              if (PARAMS_IsLightController(pSensorParams)){    // 2.4.0
                strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_ALGORITHMIC+language]);
                pValuesTemp += (strlen((char const *)pValuesTemp) + 1);
              }
              else if(sensorParams.Type.Value>=0x109 &&sensorParams.Type.Value<=0x110)
              {
                strcpy((char *)pValuesTemp, (char const *)DALI_sensor_type_name[(sensorParams.Type.Value-0x109 )+language*8]);
                pValuesTemp += (strlen((char const *)pValuesTemp) + 1);        
              }
                else{
                    if (PARAMS_IsLineVoltageSensor(pSensorParams)){
                      ...
                      if (PARAMS_IsStandaloneSensor(pSensorParams)){
                        strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_DIGITAL+language]);
                        pValuesTemp += strlen((char const *)pValuesTemp);
    //                  strcpy((char *)pValuesTemp, "DIGITAL ");
      ...
      //                  pValuesTemp += 8;
                        ...
                        strcpy((char *)pValuesTemp, (char const *)strings[STRINGS_INDEX_CENTRALIZED+language]);
                        pValuesTemp += strlen((char const *)pValuesTemp);
                      }
                    }
        ...
        if (PARAMS_IsParameterAvailable(pSensorParams, PARAM_INDEX_FIRMWAREVERSION) && DisplayReadOnlyItems)
        {
            if( is_KNX_sensor( &sensorParams ) )
            {
                (pParametersTemp)->pName = strings[STRINGS_INDEX_FIRMWARE+language];            
            }
            else
            {
                if (PARAMS_IsZigbeeSensor(pSensorParams))
    ...
    //                (pParametersTemp)->pName = "Controller firmware";
                   (pParametersTemp)->pName = strings[STRINGS_INDEX_CONTROLLER_FIM+language];
    ...
    //                (pParametersTemp)->pName = "Firmware";
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_REFERENT+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_GROUP+language];
            (pParametersTemp)->pValue = pValuesTemp;
            pParametersTemp->ValueType = 0; // string
            pParametersTemp++;
...
//            (pParametersTemp)->pName = "Dau. board firmware";
            (pParametersTemp)->pName = strings[STRINGS_INDEX_DAUGHTER_BOARD+language];
        ...
        {
            (pParametersTemp)->pName = strings[STRINGS_INDEX_BATTERY+language];
...
void _draw_rectangular_low_for_value(void)
{
  unsigned char  PAGE_VALUE_Y_POs      =          85;
    unsigned char PAGE_RECTANGULAR_LENGTh     =    108;
      unsigned char PAGE_RECTANGULAR_HEIGHt   =      20;
        unsigned char PAGE_RECTANGULAR_X_POs    =      ((128-108)/2);
          unsigned char PAGE_RECTANGULAR_Y_POs    =      85-1;
...
#define PAGE_MAIN_LINE_4                    (PAGE_MAIN_LINE_3 + PAGE_MAIN_LINE_HEIGHT + PAGE_MAIN_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        {
            pMenuItems->pString = strings[STRINGS_INDEX_BASICPARAMETERS+language];
        ...
        else
        {
            if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_HOUR) || \
                ...
                PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_BANDWIDTH)|| \
            ...
            {
                pMenuItems->pString = strings[STRINGS_INDEX_PARAMETERS+language];
                pMenuItems->PageHandlerIndex = PAGE_INDEX_PARAMETERS;
        ...
        {
            (pMenuItems+totalItems)->pString = strings[STRINGS_INDEX_ADVANCEDMODE+language];
            (pMenuItems+totalItems)->PageHandlerIndex = PAGE_INDEX_ADVANCEDMODE;
            totalItems++;
        }
        (pMenuItems+totalItems)->pString = strings[STRINGS_INDEX_FILES+language];
        ...
        if ((!PARAMS_IsZigbeeSensor(&sensorParams)) && \
          (!PARAMS_IsLightController(&sensorParams)) && !is_DALI_sensor(&sensorParams)&&!is_PRECIR_sensor(&sensorParams) && !is_KNX_sensor(&sensorParams))     // 2.4.0
        {
          (pMenuItems+totalItems)->pString = strings[STRINGS_INDEX_PNL+language ];
          (pMenuItems+totalItems)->PageHandlerIndex = PAGE_INDEX_PNLENTRY;
          totalItems++;
        }

        (pMenuItems+totalItems)->pString = strings[STRINGS_INDEX_TEST+language];        // 2.4.0
        (pMenuItems+totalItems)->PageHandlerIndex = PAGE_INDEX_TEST;
        totalItems++;
        (pMenuItems+totalItems)->pString = strings[STRINGS_INDEX_SETTINGS+language];
...
#define PAGE_READPARAMS_PGBAR_STEP      ( 2 ) // a step of the progressbar of reading all parameter; only for displaying 
    ...
    if (pageInitialize)
    {
        ...
        yOffset = PAGE_READPARAMS_BODY_Y_START+LCDD_PutString(PAGE_READPARAMS_BODY_X_START, PAGE_READPARAMS_BODY_Y_START, PAGE_READPARAMS_BODY_X_LENGTH, strings[STRINGS_INDEX_WAITREADING+language], 3, 1)+PAGE_READPARAMS_BODY_INDENT;
        LCDD_ClearRect(PAGE_READPARAMS_RECT_X_START, PAGE_READPARAMS_RECT_Y_START, PAGE_READPARAMS_RECT_X_END, yOffset, 3);
        LCDD_PutString(PAGE_READPARAMS_BODY_X_START, PAGE_READPARAMS_BODY_Y_START, PAGE_READPARAMS_BODY_X_LENGTH, strings[STRINGS_INDEX_WAITREADING+language], 3, 0);
    ...
    if (pageUpdate)
    {
        ...
        else
        {
                ...
                {
                    if (result = IR_ReadSensorParameter(&sensorParams, currentParameter) &&  PARAMS_GetParameterNumber(&sensorParams, currentParameter) != PARAM_NUMBER_ERROR)
                    {
                        retry = 0;
                        switch(result)
                        {
...
#define PAGE_TIMEZONE_MINUTE_X_POS      80
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_GMT+language]), 0, 0, strings[STRINGS_INDEX_GMT+language], 0, 0);
...
#define PAGE_FILES_LINE_4            (PAGE_FILES_LINE_3 + PAGE_FILES_LINE_HEIGHT + PAGE_FILES_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_SAVEFILE+language]), 0, 0, strings[STRINGS_INDEX_SAVEFILE+language], 0, 0);
...
//        currentItem = 0;
        ...
        if (newFileAvailable)
        {
            strcpy((char *)(pFileParams[totalItems-1].pFileName), (char const *)strings[STRINGS_INDEX_NEWFILE+language]);
        }
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_MAJOR)
        {
            if ((index > 0) || (*pCurrent != '\0'))
            {
                ...
                if (argForSubPage2 == 0)    // Indicating to save a new file
                {
                    ...
                    {
                        yEnd = PAGE_NEWFILE_BODY_Y_START+LCDD_PutString(PAGE_NEWFILE_BODY_X_START, PAGE_NEWFILE_BODY_Y_START, PAGE_NEWFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_FILESAVED+language], 3, 1);
                        ...
                        LCDD_PutString(PAGE_NEWFILE_BODY_X_START, PAGE_NEWFILE_BODY_Y_START, PAGE_NEWFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_FILESAVED+language], 3, 0);
                        LCDD_Refresh();     // Have to do this here.
                        TIME_Wait(2000);
                    }
                    pageCurrent = PAGE_INDEX_SAVEFILE;
...
#define PAGE_REPLACEFILE_BODY_Y_START   (PAGE_REPLACEFILE_RECT_Y_START+PAGE_REPLACEFILE_BODY_INDENT)
    ...
    if (pageInitialize)
    {
        ...
        yEnd = PAGE_REPLACEFILE_BODY_Y_START+LCDD_PutString(PAGE_REPLACEFILE_BODY_X_START, PAGE_REPLACEFILE_BODY_Y_START, PAGE_REPLACEFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_REPLACEFILE+language], 3, 1);
        ...
        LCDD_PutString(PAGE_REPLACEFILE_BODY_X_START, PAGE_REPLACEFILE_BODY_Y_START, PAGE_REPLACEFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_REPLACEFILE+language], 3, 0);

        memset(pageFunckeyItems, 0, sizeof(pageFunckeyItems));
        pageFunckeyItems[1].pItem = (void *)strings[STRINGS_INDEX_YES+language];
        pageFunckeyItems[1].itemType = 1;
        pageFunckeyItems[2].pItem = (void *)strings[STRINGS_INDEX_NO_1+language];
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED))
    {
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F2)
        {
            ...
            {
                yEnd = PAGE_REPLACEFILE_BODY_Y_START+LCDD_PutString(PAGE_REPLACEFILE_BODY_X_START, PAGE_REPLACEFILE_BODY_Y_START, PAGE_REPLACEFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_FILESAVED+language], 3, 1);
                LCDD_ClearRect(PAGE_REPLACEFILE_RECT_X_START, PAGE_REPLACEFILE_RECT_Y_START, PAGE_REPLACEFILE_RECT_X_END, yEnd+PAGE_REPLACEFILE_BODY_INDENT, 3);
                LCDD_PutString(PAGE_REPLACEFILE_BODY_X_START, PAGE_REPLACEFILE_BODY_Y_START, PAGE_REPLACEFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_FILESAVED+language], 3, 0);
                LCDD_Refresh();     // Have to do this here.
                TIME_Wait(2000);
            }
            pageCurrent = PAGE_INDEX_SAVEFILE;
...
{
#define PAGE_PARAMETERS_MAXIMUMITEMS    15      // 1.0.24, was 6
...
#define PAGE_PARAMETERS_LINE_4          (PAGE_PARAMETERS_LINE_3 + PAGE_PARAMETERS_LINE_HEIGHT + PAGE_PARAMETERS_LINE_SPACE)
    ...
    //PAGE_PARAMETER   pageParamer;
       static PAGE_PARAMETER *pageParamer; 
    ...
    if (pageInitialize)
    {
        ...
        pageParamer = (PAGE_PARAMETER*)malloc(sizeof(PAGE_PARAMETER));
        pageParamer->ParamIndex = PARAM_INDEX_KNX_MASTER_SLAVE;
...
//        {
//            pTempParams->pString = strings[STRINGS_INDEX_DATE+language];
...
//        }
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_HOUR) || PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_MINUTE) || PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_SECOND))
        {
            ...
            {
                pTempParams->pString = strings[STRINGS_INDEX_HOUR+language];
            }
            else
            {
                pTempParams->pString = strings[STRINGS_INDEX_TIMEDELAY+language];
            }
        ...
        {
            pTempParams->pString = strings[STRINGS_INDEX_SENSITIVITY+language];
        ...
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DAYLIGHTSETPOINT))
        {
            if (PARAMS_IsLightController(&sensorParams))    // 2.4.0
            {
                pTempParams->pString = strings[STRINGS_INDEX_SETPOINT+language];
            }
            else
            {
                pTempParams->pString = strings[STRINGS_INDEX_DAYLIGHTSETPOINT+language];
            }
        ...
        if (PARAMS_IsDALI_MASTER_DL_PIR_US(&sensorParams) || PARAMS_IsDALI_MASTER_DL_OP_PIR_US(&sensorParams)|| PARAMS_IsDALI_MASTER_OP_PIR_US(&sensorParams)) 
        {
          ...
          {
            pTempParams->pString = "DALI Offset";
        ...
        {  if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DetectionLed))
         {
            pTempParams->pString = DALI_DetectionLED_name[language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_Detection_LED;
            pTempParams++;  
          }
        }
        if (PARAMS_IsDALI_MASTER_DL_PIR_US(&sensorParams)||PARAMS_IsDALI_MASTER_DL_OP_PIR_US(&sensorParams)||PARAMS_IsDALI_MASTER_DL_OP_PIR(&sensorParams)||PARAMS_IsDALI_MASTER_OP_PIR(&sensorParams)||PARAMS_IsDALI_MASTER_PIR_DL(&sensorParams)||PARAMS_IsDALI_MASTER_OP_PIR_US(&sensorParams)||PARAMS_IsDALI_MASTER_OP_PIR(&sensorParams))  
        {
          if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_SensorConfig))
          {
            pTempParams->pString = DALI_sensorConfig_name[language];
        ...
        {
            pTempParams->pString = strings[STRINGS_INDEX_MODE+language];
        ...
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DETECTIONSCHEME) || 
            (    PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_INITIAL) 
              ...
              || PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_RETRIGGER)))
        {
            pTempParams->pString = strings[STRINGS_INDEX_DETECTIONSCHEME+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_DETECTIONSCHEME;
            pTempParams++;
        }
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_ALERT))
        {
            pTempParams->pString = strings[STRINGS_INDEX_ALERT+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_ALERT;
            pTempParams++;
        }
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_STANDBYLEVEL))
        {
            pTempParams->pString = strings[STRINGS_INDEX_STANDBYLEVEL+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_STANDBYLEVEL;
            pTempParams++;
        }
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_STANDBYDELAY))
        {
            pTempParams->pString = strings[STRINGS_INDEX_STANDBYDELAY+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_STANDBYDELAY;
            pTempParams++;
        }
        
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DIM_R) && PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DIM_F))
        {
          pTempParams->pString = strings_dim_slop_titile[language];
          pTempParams->PageHandlerIndex = PAGE_INDEX_DIMSLOPE;
          pTempParams++;
        }
        
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_AUX_MODE) && PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_AUX_PULE))
        {
          pTempParams->pString = strings_aux_input_titile[language];
          pTempParams->PageHandlerIndex = PAGE_INDEX_AUXINPUT;
          pTempParams++;
        }
        
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_ON_LEVEL))
        {
          pTempParams->pString = strings_on_level_titile[language];
          pTempParams->PageHandlerIndex = PAGE_INDEX_ONLEVEL;
          pTempParams++;
        }
                if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_YEAR) && PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_MONTH) && PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DAY))
        {
          ...
          if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_REAL_HOUR) && PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_REAL_MIN) && PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_REAL_SEC))
        {
          pTempParams->pString = strings[STRINGS_INDEX_HOUR+language];//"hour";
          pTempParams->PageHandlerIndex = PAGE_INDEX_HOUR;
          pTempParams++;
        }
        ...
        {
          pTempParams->pString = strings[STRINGS_INDEX_SUMMERHOUR+language];
          pTempParams->PageHandlerIndex = PAGE_INDEX_SUMMERHOUR;
          pTempParams++;
        }
        
        
        if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_SCHEDULD_DAY))
        {
          pTempParams->pString = string_scheduler_title[language];
          pTempParams->PageHandlerIndex = PAGE_INDEX_EXTENDSCHEDULED;
          pTempParams++;
        }
        
         if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_MODE_STATE))
        {
          pTempParams->pString = string_extended_mode_title[language];
          pTempParams->PageHandlerIndex = PAGE_INDEX_EXTENDMODE;
          pTempParams++;
        }
        
        if (PARAMS_IsDALI_MASTER_DL_PIR_US(&sensorParams) || PARAMS_IsDALI_MASTER_DL_OP_PIR_US(&sensorParams) || PARAMS_IsDALI_MASTER_OP_PIR_US(&sensorParams) || \
          PARAMS_IsDALI_MASTER_DL_OP_PIR(&sensorParams) || \
            PARAMS_IsDALI_MASTER_OP_PIR(&sensorParams) || PARAMS_IsDALI_MASTER_PIR_DL(&sensorParams) || PARAMS_IsDALI_SLAVE_OP_PIR_US(&sensorParams) || PARAMS_IsDALI_SLAVE_OP_PIR(&sensorParams) || is_PRECIR_sensor(&sensorParams))
        {
        
          pTempParams->pString = DALI_ReturnToFactory_name[language];
          pTempParams->PageHandlerIndex = PAGE_INDEX_FACTORY;
          pTempParams++;

        }
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_SENDCONDITION))
        {
            pTempParams->pString = strings[STRINGS_INDEX_SENDCONDITION+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_SENDCONDITION;
            pTempParams++;
        }
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DEADBAND))
        {
            pTempParams->pString = strings[STRINGS_INDEX_DEADBAND+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_DEADBAND;
            pTempParams++;
        }
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_TIMEBASE))
        {
            pTempParams->pString = strings[STRINGS_INDEX_TIMEBASE+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_TIMEBASE;
            pTempParams++;
        }
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_BANDWIDTH))
        {
            pTempParams->pString = strings[STRINGS_INDEX_BANDWIDTH+language];
...
//        }
        ...
        {
            pTempParams->pString = strings[STRINGS_INDEX_COUNTRY+language];
        ...
        {
            pTempParams->pString = strings[STRINGS_INDEX_GMT+language];
        ...
        if(((pPageParams+currentItem)->PageHandlerIndex)==PAGE_INDEX_FACTORY)
        {
          //
...
//            LCDD_PutString(PAGE_LINE_INDENT, PAGE_PARAMETERS_LINE_4, 0, (pPageParams+firstDisplayedItem+3)->pString, (currentItem - firstDisplayedItem == 3) ? PAGE_HIGHLIGHT_COLOR:0, 0);
            ...
            if(((pPageParams+currentItem)->PageHandlerIndex)!=PAGE_INDEX_FACTORY)
              ...
              LCDD_ClearRect(PAGE_ARROW_NAV_X_POS, PAGE_PARAMETERS_LINE_4, LCD_X_LENGTH-1, PAGE_PARAMETERS_LINE_4+PAGE_PARAMETERS_LINE_HEIGHT-1, (currentItem - firstDisplayedItem == 3) ? PAGE_HIGHLIGHT_COLOR:0);
        }
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED))
    {
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_UP  && flag==0)
        {
            if (currentItem == 0)
            {
                currentItem = totalItems-1;
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_DOWN  && flag==0)
        {
            if (currentItem == totalItems-1)
            {
                currentItem = 0;
        ...
        else if(BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
        {
          if(((pPageParams+currentItem)->PageHandlerIndex)==PAGE_INDEX_FACTORY)
          {
           ...
           if(flag==0)
            {
              {          
                WaringBlock(DALI_waring_name[language],DALI_waring_long_name[language]);
                flag ++ ;
              }  
            }
            else
            {
              IR_PnLGroup(NITOO_LM058A_NODE, CODE_START_GROUP_PNL, 0x37);
              LCDD_ClearRect(PAGE_READPARAMS_RECT_X_START, PAGE_READPARAMS_RECT_Y_START, PAGE_READPARAMS_RECT_X_END, 90, 3);
              if(_page_sub_read_parameters((pageParamer), 1))
              {   
                currentItem = PARAMS_GetParameterValue(&sensorParams,PARAM_INDEX_KNX_MASTER_SLAVE);
                if(currentItem==1)
                { 
                  flag --;
                  pageSwap =  1;
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F4  && flag==0)
        {
...
//        BUTTON_Acknowledge();
    ...
    // 2.4.0
    if (TIME_GetTime() >= pageTimeStamp &&flag==0)
    {
        pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
        pageUpdate = 1;
    }
...
#define PAGE_ADVANCEDMODE_LINE_4          (PAGE_ADVANCEDMODE_LINE_3 + PAGE_ADVANCEDMODE_LINE_HEIGHT + PAGE_ADVANCEDMODE_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        {
            pTempParams->pString = strings[STRINGS_INDEX_CALIBRATION+language];
        ...
        {
            pTempParams->pString = strings[STRINGS_INDEX_LIGHTREGULATION+language];
        ...
        {
            pTempParams->pString = strings[STRINGS_INDEX_PROVISIONOFLIGHT+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_PROVISIONOFLIGHT;
            pTempParams++;
        }
        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_LOOPTYPE))
        {
            pTempParams->pString = strings[STRINGS_INDEX_LOOPTYPE+language];
...
{
#ifdef ROOM_CONTROL
#define PAGE_HIDDEN_MAXIMUMITEMS    10
#else
...
//        if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DAYLIGHTFACTOR))
//        {
            pTempParams->pString = strings[STRINGS_INDEX_DAYLIGHTFACTOR+language];
...
//        {
            pTempParams->pString = strings[STRINGS_INDEX_NATURALLIGHTFACTOR+language];
...
//        {
            pTempParams->pString = strings[STRINGS_INDEX_DAYLIGHTFACTORCEN+language];
...
//        {
            pTempParams->pString = strings[STRINGS_INDEX_DAYLIGHTLEVEL+language];
...
//        {
            pTempParams->pString = strings[STRINGS_INDEX_SENSORSTATUS+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_SENSORSTATUS;
            pTempParams++;
//        }
            
            pTempParams->pString = strings_operating_time_titile[language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_OPTIME;
            pTempParams++;
            
            pTempParams->pString = strings[STRINGS_INDEX_CURRENTPOL+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_CURRENTPOL;
            pTempParams++;
#ifdef ROOM_CONTROL
        pTempParams->pString = strings[STRINGS_INDEX_ROOMCONTROL+language];
        pTempParams->PageHandlerIndex = PAGE_INDEX_ROOMCONTROL;
        pTempParams++;
#endif
            pTempParams->pString = strings[STRINGS_INDEX_OTHERPARAMETERS+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_OTHERPARAMETERS;
            pTempParams++;

          
            pTempParams->pString = strings[STRINGS_INDEX_DEVICE_ID+language];
...
#define PAGE_DATE_YEAR_X_POS        76
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DATE+language]), 0, 0, strings[STRINGS_INDEX_DATE+language], 0, 0);
        //if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_WEEK_DAY))  
          LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1,strings_week_day_title[language]), 38+38-8, 0, strings_week_day_title[language], 0, 0);
        
        pParameter = (PAGE_PARAMETER *)malloc(4*sizeof(PAGE_PARAMETER));
        ...
        (pParameter+3)->ParamIndex = PARAM_INDEX_WEEK_DAY;
        (pParameter+3)->Max = 7;
        (pParameter+3)->Min = 1;
        ...
        if (pageAvailability)
        {
            ...
            //if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_WEEK_DAY)) 
              _draw_rectangular_for_value();
            //if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_WEEK_DAY)) 
              _draw_rectangular_low_for_value();
    ...
    if (pageUpdate)
    {
        ...
        //LCDD_PutString(52, 85, 0,"monday", (currentItem == 3)?0:PAGE_HIGHLIGHT_COLOR, 0);
       // LCDD_ClearRect(52-15, 85,52-15+25, 85+8, 0);
        //        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);
        LCDD_ClearRect(20, 85-0, PAGE_ARROW_NAV_X_POS-1-15, PAGE_PARAMETERS_LINE_4+PAGE_PARAMETERS_LINE_HEIGHT-1+5-2+1, PAGE_HIGHLIGHT_COLOR);        
        if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_WEEK_DAY))         
         LCDD_PutString(52-15, 86, 0,find_week_num(PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_WEEK_DAY),language), (currentItem == 3)?0:PAGE_HIGHLIGHT_COLOR, 0);       
        else
          LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, " N/A "), 86, 0, " N/A ", 0, 0);
        pageUpdate = 0;
         if(currentItem==3)
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        ...
        else if ((BUTTON_GetButtonIndex() == BUTTON_INDEX_F2) || pageForceRead)
        {
            ...
            {
              if(currentItem<=2)
                _page_sub_read_parameters(pParameter, PAGE_DATE_TOTALITEMS-1);
 
              if(currentItem==3)
                _page_sub_read_parameters(pParameter+currentItem, 1);
...
//                pageUpdate = 1;
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
        {
            ...
            {
              if(currentItem<=2)
                _page_sub_set_parameters(pParameter, PAGE_DATE_TOTALITEMS-1);
...
{
#define PAGE_DATE_TOTALITEMS        (5+6+1)
#define PAGE_DATE_BODY_Y_POS        PAGE_VALUE_Y_POS
#define PAGE_DATE_BODY_X_POS        43
#define PAGE_DATE_DAY_X_POS         28
#define PAGE_DATE_MONTH_X_POS       52
#define PAGE_DATE_YEAR_X_POS        76
  #define PAGE_SUMMERHOUR_CURSORFLASH        500
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_SCHEDULD_DAY);

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DATE+language]), 0, 0, strings[STRINGS_INDEX_DATE+language], 0, 0);
        LCDD_PutString(20, 16, 0, string_scheduled_start_titile[language], 0, 0);
        LCDD_PutString(84, 16, 0,string_scheduled_stop_titile[language], 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, string_scheduled_day_titile[language]), 38+38-8, 0,string_scheduled_day_titile[language], 0, 0);
        if (pageAvailability)
        {
            _param_page_funckey_initializer(FUNCKEY_READ_WRITE);
            ...
            pParameter = (PAGE_PARAMETER *)malloc(5*sizeof(PAGE_PARAMETER));

            pParameter->ParamIndex = PARAM_INDEX_EXT_SCHEDULD_DAY;
            pParameter->Max = 127;
            pParameter->Min = 0;

            (pParameter+1)->ParamIndex = PARAM_INDEX_EXT_START_HOUR;
            (pParameter+1)->Max = 23;
            (pParameter+1)->Min = 0;

            (pParameter+2)->ParamIndex = PARAM_INDEX_EXT_START_MIN;
            (pParameter+2)->Max = 59;
            (pParameter+2)->Min = 0;
            
            (pParameter+3)->ParamIndex = PARAM_INDEX_EXT_STOP_HOUR;
            (pParameter+3)->Max = 23;
            (pParameter+3)->Min = 0;
            
            (pParameter+4)->ParamIndex = PARAM_INDEX_EXT_STOP_MIN;
            (pParameter+4)->Max = 59;
            (pParameter+4)->Min = 0;
           ...
           // _draw_rectangular_for_value();
           // _draw_rectangular_low_for_value();
            LCDD_PutString(16, PAGE_DATE_BODY_Y_POS-1, 0, "            ", PAGE_HIGHLIGHT_COLOR, 0);
            LCDD_PutString(16, 53-5, 0, "            ", PAGE_HIGHLIGHT_COLOR, 0);
            
            LCDD_PutString(80, PAGE_DATE_BODY_Y_POS-1, 0, "            ", PAGE_HIGHLIGHT_COLOR, 0);
            LCDD_PutString(80, 48, 0, "            ", PAGE_HIGHLIGHT_COLOR, 0);
    ...
    if (pageUpdate)
    {
      ...
      dectoascii(&temp[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_START_HOUR), 2);
      LCDD_PutString(PAGE_DATE_DAY_X_POS-10, PAGE_DATE_BODY_Y_POS, 0, &temp[0], (currentItem == 1)?0:PAGE_HIGHLIGHT_COLOR, 0);
      dectoascii(&temp[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_START_MIN), 2);
      LCDD_PutString(30, PAGE_DATE_BODY_Y_POS, 0, "H", PAGE_HIGHLIGHT_COLOR, 0);
      LCDD_PutString(37, PAGE_DATE_BODY_Y_POS, 0, &temp[0], (currentItem == 2)?0:PAGE_HIGHLIGHT_COLOR, 0);
      
      dectoascii(&temp[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STOP_HOUR), 2);
      LCDD_PutString(82, PAGE_DATE_BODY_Y_POS, 0, &temp[0], (currentItem == 3)?0:PAGE_HIGHLIGHT_COLOR, 0);
      dectoascii(&temp[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STOP_MIN), 2);
      LCDD_PutString(94, PAGE_DATE_BODY_Y_POS, 0, "H", PAGE_HIGHLIGHT_COLOR, 0);
      LCDD_PutString(101, PAGE_DATE_BODY_Y_POS, 0, &temp[0], (currentItem == 4)?0:PAGE_HIGHLIGHT_COLOR, 0);
        
      int i=0;
      for(i;i<7;i++)
      {
        LCDD_PutString(12+i*15, 85, 0,"    ",  ((PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_SCHEDULD_DAY)>>i &0x01))?PAGE_HIGHLIGHT_COLOR:0, 0);
        LCDD_PutString(12+i*15, 90-1, 0,"    ",  ((PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_SCHEDULD_DAY)>>i &0x01))?PAGE_HIGHLIGHT_COLOR:0, 0);
        LCDD_PutString(12+i*15, 86, 0,strings_scheduled_week[0+i+7*language], ((PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_SCHEDULD_DAY)>>i &0x01))?PAGE_HIGHLIGHT_COLOR:0, 0);
      }
      
      if(currentItem>=5 && currentItem<=11)
        LCDD_PutString(12+(currentItem-5)*15, 86, 0,strings_scheduled_week[0+currentItem-5+7*language],invertCursor==1? PAGE_HIGHLIGHT_COLOR:0, 0);
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        ...
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_MAJOR && currentItem >=5 && currentItem <=11)
        {
            if (pageAvailability)
            {
              
              
              if((PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_SCHEDULD_DAY)>>(currentItem-5) &0x01))
                _adjust_param_value(pParameter, -1*func_pos2value(currentItem));
              
              else
                _adjust_param_value(pParameter, func_pos2value(currentItem));                
        ...
        else if ((BUTTON_GetButtonIndex() == BUTTON_INDEX_F2) || pageForceRead)
        {
            if (pageAvailability)
            {
                _page_sub_read_parameters(pParameter, 5);
...
//                pageUpdate = 1;
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
        {
            if (pageAvailability)
            {
                _page_sub_set_parameters(pParameter, 5);
...
//                pageUpdate = 1;
        ...
        {
            pageCurrent = PAGE_INDEX_PARAMETERS;//PAGE_INDEX_EXTENDMODE;
...
#define PAGE_TIMEDELAY_PULSE_HEIGHT 32      // 2.3.0
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_REAL_HOUR ) || PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_REAL_MIN) || PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_REAL_SEC);

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        {
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_HOUR+language]), 0, 0, strings[STRINGS_INDEX_HOUR+language], 0, 0);
        }
...
//        _draw_rectangular_for_value();    // 2.3.0
        ...
        if (pageAvailability)
        {
          ...
          pParameter = (PAGE_PARAMETER *)malloc(3*sizeof(PAGE_PARAMETER));    // 2.3.1, was 3
          
          pParameter->ParamIndex = PARAM_INDEX_EXT_REAL_HOUR;
          ...
          (pParameter+1)->ParamIndex = PARAM_INDEX_EXT_REAL_MIN;
          (pParameter+1)->Max = 59;
          (pParameter+1)->Min = 0;
          //        (pParameter+1)->Updated = 1;
          
          (pParameter+2)->ParamIndex = PARAM_INDEX_EXT_REAL_SEC;
          (pParameter+2)->Max = 59;
    ...
    if (pageUpdate)
    {
        ...
        else
        {
            ...
            if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_REAL_HOUR))
            {
                if (language == German)     // 2.4.0
                {
            ...
            if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_REAL_HOUR))   // && (pParameter->Updated))
            {

              BcdBits = 2;
              dectoascii(&pHour[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_REAL_HOUR), BcdBits);
...
//                (pParameter+1)->Updated = 0;
                dectoascii(&pMin[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_REAL_MIN        ), 2);
...
//                (pParameter+2)->Updated = 0;
                dectoascii(&pSec[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_REAL_SEC        ), 2);
...
//            pParamTemp->Updated = 1;
        ...
        else if ((BUTTON_GetButtonIndex() == BUTTON_INDEX_F2) || pageForceRead)
        {
            ...
            _page_sub_read_parameters(pParameter, 3);       // 2.3.1, was 3
...
#define PAGE_TIMEDELAY_PULSE_HEIGHT 32      // 2.3.0
    ...
    if (pageInitialize)
    {
        ...
        {
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_TIMEDELAY+language]), 0, 0, strings[STRINGS_INDEX_TIMEDELAY+language], 0, 0);
        }
...
//        _draw_rectangular_for_value();    // 2.3.0
        ...
        if (pageAvailability)
        {
            ...
            else
            {
              if (PARAMS_IsLightController(&sensorParams))    // 2.4.0
                pParameter->Max = 23;
              else  if (PARAMS_IsDALI_MASTER_DL_OP_PIR_US(&sensorParams)||PARAMS_IsDALI_MASTER_DL_OP_PIR(&sensorParams)||PARAMS_IsDALI_MASTER_OP_PIR(&sensorParams)||PARAMS_IsDALI_MASTER_OP_PIR_US(&sensorParams))
              {
                pParameter->Max = 18;
              }
              else
    ...
    //        (pParameter+2)->Updated = 1;
            if(sensorParams.Type.Value==0x107 || sensorParams.Type.Value==0x111 ||sensorParams.Type.Value==0x112||sensorParams.Type.Value==0x113)
             (pParameter+2)->Min = 5;
...
#define PAGE_SENSITIVITY_BODY_Y_POS_3       (PAGE_SENSITIVITY_BODY_Y_POS_2 + PAGE_SENSITIVITY_BODY_Y_POS_DELTA)
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_SENSITIVITY+language]), 0, 0, strings[STRINGS_INDEX_SENSITIVITY+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        for (i=0; i<totalItems; i++)
        {
            ...
            LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS, yOffset, 0, strings[STRINGS_INDEX_LOW+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, (pParameter+i)->ParamIndex)+language], 0, 0);      // 2.4.0
...
#define PAGE_CALIBRATION_BODY_Y_POS         PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_CALIBRATION+language]), 0, 0, strings[STRINGS_INDEX_CALIBRATION+language], 0, 0);
    ...
    if( DispWarningFlag ){
      if (language == German){     // 2.4.0
        if( TIME_GetTime() >= pageTimeStamp ){
          ...
          LCDD_PutString(0, 0, LCD_X_LENGTH, strings[STRINGS_INDEX_CALIBRATIONPROMPT+language]+162, 0, 0);
          DispWarningFlag = 0;
        }
      }
    }
    if (pageUpdate)
    {
        if (ConfirmPrompt)
        {
            ...
            LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);
            LCDD_PutString(0, 0, LCD_X_LENGTH, strings[STRINGS_INDEX_CALIBRATIONPROMPT+language], 0, 0);
...
#define PAGE_DAYLIGHTSETPOINT_EYE_HEIGHT    32      // 2.3.0
    ...
    if (pageInitialize)
    {
        ...
        if (PARAMS_IsLightController(&sensorParams))    // 2.4.0
        {
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_SETPOINT+language]), 0, 0, strings[STRINGS_INDEX_SETPOINT+language], 0, 0);
        }
        else
        {
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DAYLIGHTSETPOINT+language]), 0, 0, strings[STRINGS_INDEX_DAYLIGHTSETPOINT+language], 0, 0);
        }
...
//    const unsigned char *string;      // 2.3.0
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_MODE+language ]), 0, 0, strings[STRINGS_INDEX_MODE +language], 0, 0);
...
//        previousItem = 0xFF;      // 2.3.0
        ...
        if(sensorParams.Type.Value>=0x108 && sensorParams.Type.Value<=0x110)
        {
        ...
        if(is_PRECIR_sensor(&sensorParams))
        {
          ...
          if((sensorParams.Type.Value) == PARAM_TYPE_HF_PRECIR_BILEVEL)
          {
            possible_values[0]=7;
            possible_values[1]=8;
...
//                LCDD_PutString(PAGE_MODE_NAME_X_POS, rect[i-firstDisplayedItem].yStart+1, 0, strings[STRINGS_INDEX_MODE+possible_values[i]*STRINGS_TOTAL_LANGUAGES ], (i == currentItem) ? PAGE_HIGHLIGHT_COLOR:0, 0);
                if (LCDD_PutScrollString(PAGE_MODE_NAME_X_POS, rect[i-firstDisplayedItem].yStart+1, (PAGE_MODE_VALUE_X_POS-2-PAGE_MODE_NAME_X_POS), strings[STRINGS_INDEX_MODE+possible_values[i]*STRINGS_TOTAL_LANGUAGES +language]+((i == currentItem)?pageScroll:0), (i == currentItem)?PAGE_HIGHLIGHT_COLOR:0, (i == currentItem)?1:0))
                {
                    pageScroll++;
                    pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
                }
...
//    static unsigned char previousItem;    // 2.3.0
    ...
    static PAGE_PARAMETER *pParameter;
    static unsigned char mode_value;
    const unsigned char possible_values[3]={0x01,0x02,0x03};    // 2.3.0
    unsigned char i;
//    const unsigned char *string;      // 2.3.0
    const unsigned char * string[]={DALI_daylight_name[language],DALI_occupancy_name[language],DALI_occupancyDaylight_name[language]};
    if (pageInitialize)
    {
        pageInitialize = 0;
     ...
     //   LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_sensorConfig_name[language]), 0, 0, DALI_sensorConfig_name[language], 0, 0);

        if(Spanish==language)
        {
          LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_sensorEpanish_name[0]), 0, 0, DALI_sensorEpanish_name[0], 0, 0);
          LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_sensorEpanish_name1[0]), PAGE_MODE_BODY_Y_POS_1, 0, DALI_sensorEpanish_name1[0], 0, 0);
        ...
        if (pageAvailability)
        {
          ...
          (pParameter+1)->ParamIndex = PARAM_INDEX_TYPE;
          // 2.3.1, hide one more parameter Type2 in the list in order to read to determine if Pulse available to display
          // (pParameter+1)->ParamIndex = PARAM_INDEX_TYPE2;
         // possible_values[totalItems] = 3;
          totalItems=3;
          ...
          {
            if (mode_value == possible_values[i-1])
            {
              currentItem = i;
              break;
            }
    ...
    if (pageUpdate)
    {
      if(Spanish==language)
      {
        for (i=1; i<=totalItems; i++)
        {
          ...
          if (LCDD_PutScrollString(PAGE_MODE_NAME_X_POS, rect[i].yStart+1, (PAGE_MODE_VALUE_X_POS-2-PAGE_MODE_NAME_X_POS), string[i-1]+((i == currentItem)?pageScroll:0), (i == currentItem)?PAGE_HIGHLIGHT_COLOR:0, (i == currentItem)?1:0))
          {
            pageScroll++;
            pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
          }
          ...
          LCDD_ClearRect(PAGE_MODE_VALUE_X_POS-2, rect[i].yStart+1, LCD_X_LENGTH-2, rect[i].yEnd-1, 0);
          LCDD_PutBitmap(PAGE_MODE_VALUE_X_POS, rect[i].yStart+1, (mode_value == possible_values[i-1])?&BitmapInfo_Validation[1]:&BitmapInfo_Validation[0], 0);
        }
      ...
      {    
        for (i=1; i<=totalItems; i++)
        {
          LCDD_ClearRect(0, rect[i-1].yStart, LCD_X_LENGTH-1, rect[i-1].yEnd, (i == currentItem) ? PAGE_HIGHLIGHT_COLOR:0);
          if (LCDD_PutScrollString(PAGE_MODE_NAME_X_POS, rect[i-1].yStart+1, (PAGE_MODE_VALUE_X_POS-2-PAGE_MODE_NAME_X_POS), string[i-1]+((i == currentItem)?pageScroll:0), (i == currentItem)?PAGE_HIGHLIGHT_COLOR:0, (i == currentItem)?1:0))
          {
            pageScroll++;
            pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
          }
          ...
          LCDD_ClearRect(PAGE_MODE_VALUE_X_POS-2, rect[i-1].yStart+1, LCD_X_LENGTH-2, rect[i-1].yEnd-1, 0);
          LCDD_PutBitmap(PAGE_MODE_VALUE_X_POS, rect[i-1].yStart+1, (mode_value == possible_values[i-1])?&BitmapInfo_Validation[1]:&BitmapInfo_Validation[0], 0);
        }
      }
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_MAJOR)
        {
          if (pageAvailability)
          {
            mode_value = possible_values[currentItem-1];
...
#define PAGE_DETECTIONSCHEME_BODY_Y_POS_3   62
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability =     PARAMS_IsParameterAvailable( &sensorParams, PARAM_INDEX_DETECTIONSCHEME ) 
                           || (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_INITIAL) 
                                || PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_MAINTAIN) 
                                || PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_RETRIGGER));
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DETECTIONSCHEME+language]), 0, 0, strings[STRINGS_INDEX_DETECTIONSCHEME+language], 0, 0);
        ...
        if (pageAvailability)
        {
            ...
            else if(sensorParams.Type.Value>=0x108 && sensorParams.Type.Value<=0x110)
            {
              ...
              if( 0x10F == sensorParams.Type.Value )
              {
                ...
                if( 0x110 == sensorParams.Type.Value )
              {
                totalItems = 0;
                if( PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_INITIAL) )
                {
    ...
    if (pageUpdate)
    {
        ...
        if (totalItems >= 1)
        {
            ...
            {
                // 2.4.0
                if (LCDD_PutScrollString(PAGE_DETECTIONSCHEME_NAME_X_POS, PAGE_DETECTIONSCHEME_BODY_Y_POS_1, (PAGE_DETECTIONSCHEME_VALUE_X_POS-2-PAGE_DETECTIONSCHEME_NAME_X_POS), strings[STRINGS_INDEX_INITIAL+language]+((currentItem == 0)?pageScroll:0), (currentItem == 0)?PAGE_HIGHLIGHT_COLOR:0, (currentItem == 0)?1:0))
                {
                    pageScroll++;
                    pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
                }
                LCDD_PutString(PAGE_DETECTIONSCHEME_VALUE_X_POS, PAGE_DETECTIONSCHEME_BODY_Y_POS_1, 0, strings[STRINGS_INDEX_DISABLED+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_INITIAL)+language], 0, 0);      // 2.4.0
            }
            else
            {
                LCDD_PutString(PAGE_DETECTIONSCHEME_VALUE_X_POS, PAGE_DETECTIONSCHEME_BODY_Y_POS_1, 0, strings[STRINGS_INDEX_DISABLED+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DETECTIONSCHEME)+language], 0, 0);      // 2.4.0
            }
        ...
        if (totalItems >= 2)
        {
            ...
            // 2.4.0
            if (LCDD_PutScrollString(PAGE_DETECTIONSCHEME_NAME_X_POS, PAGE_DETECTIONSCHEME_BODY_Y_POS_2, (PAGE_DETECTIONSCHEME_VALUE_X_POS-2-PAGE_DETECTIONSCHEME_NAME_X_POS), strings[STRINGS_INDEX_MAINTAIN+language]+((currentItem == 1)?pageScroll:0), (currentItem == 1)?PAGE_HIGHLIGHT_COLOR:0, (currentItem == 1)?1:0))
            {
                pageScroll++;
                pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
            }
            LCDD_PutString(PAGE_DETECTIONSCHEME_VALUE_X_POS, PAGE_DETECTIONSCHEME_BODY_Y_POS_2, 0, strings[STRINGS_INDEX_DISABLED+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_MAINTAIN)+language], 0, 0);     // 2.4.0
        ...
        if (totalItems >= 3)
        {
            ...
            // 2.4.0
            if (LCDD_PutScrollString(PAGE_DETECTIONSCHEME_NAME_X_POS, PAGE_DETECTIONSCHEME_BODY_Y_POS_3, (PAGE_DETECTIONSCHEME_VALUE_X_POS-2-PAGE_DETECTIONSCHEME_NAME_X_POS), strings[STRINGS_INDEX_RETRIGGER+language]+((currentItem == 2)?pageScroll:0), (currentItem == 2)?PAGE_HIGHLIGHT_COLOR:0, (currentItem == 2)?1:0))
            {
                pageScroll++;
                pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
            }
            if (language >= Italian){      // 2.4.0, was Spanish            
              if (LCDD_PutScrollString(PAGE_DETECTIONSCHEME_VALUE_X_POS, PAGE_DETECTIONSCHEME_BODY_Y_POS_3, LCD_X_LENGTH-PAGE_DETECTIONSCHEME_VALUE_X_POS-12, strings[STRINGS_INDEX_DISABLED+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_RETRIGGER)+language]+pageScroll_1, 0, 1))
              {
                pageScroll_1++;
                pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
              }
              ...
              LCDD_PutString(PAGE_DETECTIONSCHEME_VALUE_X_POS, PAGE_DETECTIONSCHEME_BODY_Y_POS_3, 0, strings[STRINGS_INDEX_DISABLED+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_RETRIGGER)+language], 0, 0);    // 2.4.0
            }
...
#define PAGE_ALERT_BODY_Y_POS     18
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_ALERT+language]), 0, 0, strings[STRINGS_INDEX_ALERT+language], 0, 0);
        ...
        if(sensorParams.Type.Value==0x113)
        {
          LCDD_PutString(PAGE_ALERT_NAME_X_POS, PAGE_ALERT_BODY_Y_POS, 0, "Visuel", 0, 0);
        }
        else  
          LCDD_PutString(PAGE_ALERT_NAME_X_POS, PAGE_ALERT_BODY_Y_POS, 0, strings[STRINGS_INDEX_AUDIBLE+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        if (PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_ALERT) == 2)    //2 refers to audible alert
        {
          ...
          if (PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_ALERT) == 1)    //2 refers to audible alert
        {
            LCDD_PutBitmap(_figure_icon_center_pos(PAGE_ALERT_VALUE_X_POS, LCD_X_LENGTH-2, BitmapInfo_Validation[1].Width), PAGE_ALERT_BODY_Y_POS, &BitmapInfo_Validation[1], 0);
        }
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_MAJOR)
        {
          if(sensorParams.Type.Value==0x113)
          {
            if (pageAvailability)
            {
              if (PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_ALERT) == 1)
              {
                PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_ALERT, 0);
              }
              else
              {
                PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_ALERT, 1);
              }
...
#define PAGE_STANDBYLEVEL_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_STANDBYLEVEL+language]), 0, 0, strings[STRINGS_INDEX_STANDBYLEVEL+language], 0, 0);
        ...
        if (pageAvailability)
        {
            ...
            if(sensorParams.Type.Value==0x107 || sensorParams.Type.Value==0x111 || sensorParams.Type.Value==0x112)
            {
              pParameter->Max = 8;
              pParameter->Min = 4;
            }
...
#define PAGE_STANDBYDELAY_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_STANDBYDELAY+language]), 0, 0, strings[STRINGS_INDEX_STANDBYDELAY+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        if (tempvalue == 0)
        {
            strcpy((char *)&pStandbydelay[0], (char const *)strings[STRINGS_INDEX_NOSTANDBY+language]);
        }
        else if (tempvalue == 255)
        {
            strcpy((char *)&pStandbydelay[0], (char const *)strings[STRINGS_INDEX_UNLIMITED+language]);
        }
...
#define PAGE_SENDCONDITION_BODY_Y_POS_4      84
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_SENDCONDITION+language]), 0, 0, strings[STRINGS_INDEX_SENDCONDITION+language], 0, 0);
        ...
        LCDD_PutString(PAGE_SENDCONDITION_NAME_X_POS, rect[0].yStart+1, PAGE_SENDCONDITION_NAME_MAX_LENGTH, strings[STRINGS_INDEX_CYCLICAL+language], 0, 0);
        LCDD_PutString(PAGE_SENDCONDITION_NAME_X_POS, rect[1].yStart+1, PAGE_SENDCONDITION_NAME_MAX_LENGTH, strings[STRINGS_INDEX_ONCHANGE+language], 0, 0);
        LCDD_PutString(PAGE_SENDCONDITION_NAME_X_POS, rect[2].yStart+1, PAGE_SENDCONDITION_NAME_MAX_LENGTH, strings[STRINGS_INDEX_ONREQUEST+language], 0, 0);
        LCDD_PutString(PAGE_SENDCONDITION_NAME_X_POS, rect[3].yStart+1, PAGE_SENDCONDITION_NAME_MAX_LENGTH, strings[STRINGS_INDEX_CHANGEORCYCLICAL+language], 0, 0);
...
#define PAGE_DEADBAND_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DEADBAND+language]), 0, 0, strings[STRINGS_INDEX_DEADBAND+language], 0, 0);
...
#define PAGE_TIMEBASE_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_TIMEBASE+language]), 0, 0, strings[STRINGS_INDEX_TIMEBASE+language], 0, 0);
...
#define PAGE_BANDWIDTH_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_BANDWIDTH+language]), 0, 0, strings[STRINGS_INDEX_BANDWIDTH+language], 0, 0);
...
#define PAGE_SUMMERHOUR_CURSORFLASH        500
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_SUMMERHOUR+language]), 0, 0, strings[STRINGS_INDEX_SUMMERHOUR+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        LCDD_PutString(PAGE_SUMMERHOUR_GEO_X_POS, PAGE_SUMMERHOUR_GEO_Y_POS, 0, strings[STRINGS_INDEX_NOCHOICE+geo_values_idx*STRINGS_TOTAL_LANGUAGES+language], 0, 0);
        if (currentItem == 0)
        {
        ...
        if (totalItems > 1)
        {
            ...
            LCDD_PutString(_figure_string_center_x_pos(PAGE_SUMMERHOUR_RECT_GEO_X_START, LCD_X_LENGTH/2-1, strings[STRINGS_INDEX_STARTDAY+language]), PAGE_SUMMERHOUR_DAY_TEXTS_Y_POS, 0, strings[STRINGS_INDEX_STARTDAY+language], 0, 0);
            LCDD_PutString(_figure_string_center_x_pos(LCD_X_LENGTH/2, PAGE_SUMMERHOUR_RECT_GEO_X_END, strings[STRINGS_INDEX_ENDDAY+language]), PAGE_SUMMERHOUR_DAY_TEXTS_Y_POS, 0, strings[STRINGS_INDEX_ENDDAY+language], 0, 0);
        }
...
#define PAGE_COUNTRY_TEXT_LAT_Y_POS         PAGE_COUNTRY_LAT_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_COUNTRY+language]), 0, 0, strings[STRINGS_INDEX_COUNTRY+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        LCDD_PutString(PAGE_COUNTRY_CNTY_X_POS, PAGE_COUNTRY_CNTY_Y_POS, 0, valuesCountry[PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_COUNTRY)*5+language], 0, 0);
        if (currentItem == 0)
        {
...
//#define PAGE_LIGHTREGULATION_TITLE_X_POS    20
  ...
  const unsigned char *string_speed[]={" ",DALI_veryfast_name[language],DALI_fast_name[language],DALI_normal_name[language],DALI_low_name[language],DALI_verylow_name[language]};
  static PAGE_PARAMETER *pParameter;
  unsigned char speed_value;  
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_LIGHTREGULATION+language]), 0, 0, strings[STRINGS_INDEX_LIGHTREGULATION+language], 0, 0);
...
//          LCDD_InvertRect(0, PAGE_LIGHTREGULATION_BODY_Y_POS-1, LCD_X_LENGTH-1, PAGE_LIGHTREGULATION_BODY_Y_POS+16, PAGE_HIGHLIGHT_COLOR);
        ...
        { 
          ...
          //LCDD_PutString(0, 18+22, 0, DALI_ReactionSpeed_name[language], 0, 0);//2016.4.15
          // LCDD_PutString(0, 40, 0, (pStrings+firstDisplayedItem*PAGE_SENSORSTATUS_STRING_MAX_LENGTH), (currentItem - firstDisplayedItem == 0) ? 3:0, 0);
          //  LCDD_InvertRect(0, rect[currentItem].yStart, LCD_X_LENGTH-1, rect[currentItem].yEnd, PAGE_HIGHLIGHT_COLOR); 
           LCDD_PutScrollString(0, 40, 70-5, DALI_ReactionSpeed_name[language], 0, 0); 
          currentItem_x=PARAMS_GetParameterValue(&sensorParams, (pParameter+1)->ParamIndex);    
          LCDD_ClearRect(40+40-4, 18+22, LCD_X_LENGTH-2-80, 18+22+15, 0);
          if(currentItem_x>=1 && currentItem_x<=5 )
    ...
    if (pageUpdate)
    {
       if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_LIGHTREGULATION))
        {
          ...
          LCDD_PutString(2, PAGE_LIGHTREGULATION_BODY_Y_POS, 0, strings[STRINGS_INDEX_LIGHTREGULATION+language ], 0, 0);
        ...
        if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_ReactionSpeed))
      {
        ...
        if(currentItem==0)
        {
          ...
          LCDD_ClearRect(0, rect[0].yStart, LCD_X_LENGTH-1, rect[0].yEnd, 0);
          //LCDD_PutString(0, 18+22, 0, DALI_ReactionSpeed_name[language], 0, 0);         
          LCDD_PutScrollString(0, 40, 70-5, DALI_ReactionSpeed_name[language], 0, 0); 
          LCDD_PutString(40+40-5-3-5, 18+22, 0, string_speed[currentItem_x], 0, 0);      // 2.4.0
          // LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, 18+22, 0, "<>", 0, 0);  
          pageScroll=0;
        }
        if(currentItem==1)
        {
          ...
          LCDD_PutString(PAGE_LIGHTREGULATION_NAME_X_POS, PAGE_LIGHTREGULATION_BODY_Y_POS, 0, strings[STRINGS_INDEX_LIGHTREGULATION+language ], 0, 0);
          LCDD_ClearRect(0, rect[0].yStart, LCD_X_LENGTH-1, rect[0].yEnd, 0);
          
         // LCDD_PutString(0, 18+22, 0, DALI_ReactionSpeed_name[language], 0, 0); 
          if (LCDD_PutScrollString(0, 40, 70-5, DALI_ReactionSpeed_name[language]+pageScroll, 0, 1))
          {
            pageScroll++;
            pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
          }
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
      if (BUTTON_GetButtonIndex() == BUTTON_INDEX_MAJOR && currentItem==0)
      {
        if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_LIGHTREGULATION)) 
        {
        ...
        else if ((BUTTON_GetButtonIndex() == BUTTON_INDEX_F2) || pageForceRead)
        {
          ...
          if (pageAvailability)
          {
            if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_ReactionSpeed))
            {
              ...
              { 
                _page_sub_read_parameters(pParameter, 2);
                currentItem_x=PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_ReactionSpeed);
              }
              if(currentItem==0)
                _page_sub_read_parameters(pParameter, 1);   
            }else
        ...
        {
            if(PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_ReactionSpeed)&& currentItem==1)
            {
             if (currentItem_x == 1)
            {
                currentItem_x = 5;
...
#define PAGE_PROVISIONOFLIGHT_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_PROVISIONOFLIGHT+language]), 0, 0, strings[STRINGS_INDEX_PROVISIONOFLIGHT+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        {
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_AUTO+language]), PAGE_PROVISIONOFLIGHT_BODY_Y_POS, 0, strings[STRINGS_INDEX_AUTO+language], PAGE_HIGHLIGHT_COLOR, 0);       // 2.4.0
        }
...
#define PAGE_LOOPTYPE_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_LOOPTYPE+language]), 0, 0, strings[STRINGS_INDEX_LOOPTYPE+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_CLOSELOOP+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_LOOPTYPE)+language])+1, PAGE_LOOPTYPE_BODY_Y_POS, 0, strings[STRINGS_INDEX_CLOSELOOP+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_LOOPTYPE)+language], PAGE_HIGHLIGHT_COLOR, 0);
...
#define PAGE_DAYLIGHTFACTOR_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DAYLIGHTFACTOR+language]), 0, 0, strings[STRINGS_INDEX_DAYLIGHTFACTOR+language], 0, 0);
...
#define PAGE_DAYLIGHTFACTORCEN_BODY_Y_POS   PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DAYLIGHTFACTORCEN+language]), 0, 0, strings[STRINGS_INDEX_DAYLIGHTFACTORCEN+language], 0, 0);
...
#define PAGE_NATURALLIGHTFACTOR_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_NATURALLIGHTFACTOR+language]), 0, 0, strings[STRINGS_INDEX_NATURALLIGHTFACTOR+language], 0, 0);
...
#define PAGE_DAYLIGHTLEVEL_BODY_Y_POS         PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DAYLIGHTLEVEL+language]), 0, 0, strings[STRINGS_INDEX_DAYLIGHTLEVEL+language], 0, 0);
...
//    static const PAGE_ITEM_RECT rect[4] = {{PAGE_SENSORSTATUS_BODY_Y_POS_1-1, PAGE_SENSORSTATUS_BODY_Y_POS_1+16}, {PAGE_SENSORSTATUS_BODY_Y_POS_2-1, PAGE_SENSORSTATUS_BODY_Y_POS_2+16}, {PAGE_SENSORSTATUS_BODY_Y_POS_3-1, PAGE_SENSORSTATUS_BODY_Y_POS_3+16}, {PAGE_SENSORSTATUS_BODY_Y_POS_4-1, PAGE_SENSORSTATUS_BODY_Y_POS_4+16}};
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_SENSORSTATUS+language]), 0, 0, strings[STRINGS_INDEX_SENSORSTATUS+language], 0, 0);
        ...
        if (data_available)
        {
            ...
            index = 0;
            strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_LOAD+language]);
            index += strlen((const char *)&pStrings[index]);
            pStrings[index++] = ' ';
            switch(temp)
            {
            case 0:
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_OFF+language]);      // 2.4.0
                break;
            case 100:
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_ON+language]);       // 2.4.0
                break;
            default:
            ...
            strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_SUNDETECTION+language]);
            index += strlen((const char *)&pStrings[index]);
            pStrings[index++] = ' ';
            if (temp & 0x0080)
            {
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_ACTIVE+language]);
            }
            else
            {
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_INACTIVE+language]);
            }
            ...
            if (temp & 0x0040)
            {
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_STANDBYMODE+language]);
            }
            else
            {
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_NOSTANDBY+language]);
            }
            ...
            {
//                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_ACTIVE+language]);
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_REGULATION_U+language]);
            }
            else
            {
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_NO_2+language]);
                index += strlen((const char *)&pStrings[index]);
                pStrings[index++] = ' ';
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_REGULATION_L+language]);
            }
            ...
            {
//                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_ACTIVE+language]);
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_DEROGATION_U+language]);
            }
            else
            {
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_NO_2+language]);
                index += strlen((const char *)&pStrings[index]);
                pStrings[index++] = ' ';
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_DEROGATION_L+language]);
            }
            ...
            {
//                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_ACTIVE+language]);
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_DETECTION_U+language]);
            }
            else
            {
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_NO_2+language]);
                index += strlen((const char *)&pStrings[index]);
                pStrings[index++] = ' ';
                strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_DETECTION_L+language]);
            }

            // fill pStrings[5][0] ~ pStrings[5][PAGE_SENSORSTATUS_STRING_MAX_LENGTH-1]
            index = 6*PAGE_SENSORSTATUS_STRING_MAX_LENGTH;
            strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_TIMEDELAY+language]);
            index += strlen((const char *)&pStrings[index]);
            pStrings[index++] = ' ';
            if (temp & 0x0004)
            {
            ...
            strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_WALKTHROUGH+language]);
            index += strlen((const char *)&pStrings[index]);
            pStrings[index++] = ' ';
            if (temp & 0x0002)
            {
            ...
            strcpy((char *)&pStrings[index], (char const *)strings[STRINGS_INDEX_RETRIGGER+language]);
            index += strlen((const char *)&pStrings[index]);
            pStrings[index++] = ' ';
            if (temp & 0x0001)
            {
...
#define PAGE_CURRENTPOL_BODY_Y_POS         PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        // 2.4.0
//        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_CURRENTPOL+language]), 0, 0, strings[STRINGS_INDEX_CURRENTPOL+language], 0, 0);
...
//        _draw_rectangular_for_value();
    ...
    if (pageUpdate)
    {
        ...
        // 2.4.0
        if (LCDD_PutScrollString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_CURRENTPOL+language]), 0, LCD_X_LENGTH, strings[STRINGS_INDEX_CURRENTPOL+language]+pageScroll, 0, 1))
        {
            pageScroll++;
            pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
        }
...
#define PAGE_OTHERPARAMETERS_LINE_4          (PAGE_OTHERPARAMETERS_LINE_3 + PAGE_OTHERPARAMETERS_LINE_HEIGHT + PAGE_OTHERPARAMETERS_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        pTempParams->pString = strings[STRINGS_INDEX_BUZZERFEEDBACK+language];
        pTempParams->PageHandlerIndex = PAGE_INDEX_BUZZERFEEDBACK;
        pTempParams++;

        pTempParams->pString = strings[STRINGS_INDEX_LOADTYPE+language];
        pTempParams->PageHandlerIndex = PAGE_INDEX_LOADTYPE;
        pTempParams++;

        pTempParams->pString = strings[STRINGS_INDEX_POWERLINEFILTER+language];
...
#define PAGE_BUZZERFEEDBACK_BODY_Y_POS  18
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_BUZZERFEEDBACK+language]), 0, 0, strings[STRINGS_INDEX_BUZZERFEEDBACK+language], 0, 0);
        ...
        if (pageAvailability)
        {
            ...
            LCDD_PutString(PAGE_BUZZERFEEDBACK_NAME_X_POS, PAGE_BUZZERFEEDBACK_BODY_Y_POS, 0, strings[STRINGS_INDEX_BUZZERFEEDBACK+language], 0, 0);
...
void LoadtypePageHandler(void)
{
    ...
    if (pageInitialize)
    {
     pageScroll = 0;                    
      ...
      pageInitialize = 0;
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_LOADTYPE+language]), 0, 0, strings[STRINGS_INDEX_LOADTYPE+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        ptmpstr = strings[STRINGS_INDEX_LOADTYPE+STRINGS_TOTAL_LANGUAGES*((unsigned char)PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_LOADTYPE)+1)+language];
        xpos = _figure_string_center_x_pos(PAGE_RECTANGULAR_X_POS+3, PAGE_RECTANGULAR_X_POS+PAGE_RECTANGULAR_LENGTH+2, strings[STRINGS_INDEX_LOADTYPE+STRINGS_TOTAL_LANGUAGES*((unsigned char)PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_LOADTYPE)+1)+language]);
...
#define PAGE_POWERLINEFILTER_BODY_Y_POS  18
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_POWERLINEFILTER+language]), 0, 0, strings[STRINGS_INDEX_POWERLINEFILTER+language], 0, 0);
        ...
        if (pageAvailability)
        {
            ...
            LCDD_PutString(PAGE_POWERLINEFILTER_NAME_X_POS, PAGE_POWERLINEFILTER_BODY_Y_POS, 0, strings[STRINGS_INDEX_POWERLINEFILTER+language], 0, 0);
...
//#define PAGE_FILES_LINE_4            (PAGE_FILES_LINE_3 + PAGE_FILES_LINE_HEIGHT + PAGE_FILES_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_FILES+language]), 0, 0, strings[STRINGS_INDEX_FILES+language], 0, 0);

        FILES_ReadFAT(&fat);
        if(is_PRECIR_sensor(&sensorParams))
        {
          totalItems = FILES_GetFileNumber(&fat);   // plus 1 for default files
          currentItem = 0;
        }
        else
...
//        }
        ...
        else
        {
          ...
          {
            LCDD_ClearRect(0, PAGE_FILES_LINE_1, LCD_X_LENGTH-1, PAGE_FILES_LINE_1+PAGE_FILES_LINE_HEIGHT-1, (currentItem - firstDisplayedItem == 0) ? 3:0);
            if (firstDisplayedItem == 0 && !is_PRECIR_sensor(&sensorParams))    // to display "Default files"
            {
              LCDD_PutString(PAGE_LINE_INDENT, PAGE_FILES_LINE_1, 0, strings[STRINGS_INDEX_DEFAULTFILES+language], (currentItem - firstDisplayedItem == 0) ? 3:0, 0);
              LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_FILES_LINE_1, (currentItem - firstDisplayedItem == 0) ? &BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
            }
            else
            {
...
#define PAGE_DEFAULTFILES_TOTALITEMS    12
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DEFAULTFILES+language]), 0, 0, strings[STRINGS_INDEX_DEFAULTFILES+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        {
            LCDD_ClearRect(0, PAGE_FILES_LINE_1, LCD_X_LENGTH-1, PAGE_FILES_LINE_1+PAGE_FILES_LINE_HEIGHT-1, (currentItem - firstDisplayedItem == 0) ? 3:0);
            LCDD_PutString(PAGE_LINE_INDENT, PAGE_FILES_LINE_1, 0, strings[STRINGS_INDEX_CORRIDORHALLAUTO+STRINGS_TOTAL_LANGUAGES*firstDisplayedItem+language], (currentItem - firstDisplayedItem == 0) ? 3:0, 0);
        }

        if (PAGE_DEFAULTFILES_TOTALITEMS >= 2)
        {
            LCDD_ClearRect(0, PAGE_FILES_LINE_2, LCD_X_LENGTH-1, PAGE_FILES_LINE_2+PAGE_FILES_LINE_HEIGHT-1, (currentItem - firstDisplayedItem == 1) ? 3:0);
            LCDD_PutString(PAGE_LINE_INDENT, PAGE_FILES_LINE_2, 0, strings[STRINGS_INDEX_CORRIDORHALLAUTO+STRINGS_TOTAL_LANGUAGES*(firstDisplayedItem+1)+language], (currentItem - firstDisplayedItem == 1) ? 3:0, 0);
        }

        if (PAGE_DEFAULTFILES_TOTALITEMS >= 3)
        {
            LCDD_ClearRect(0, PAGE_FILES_LINE_3, LCD_X_LENGTH-1, PAGE_FILES_LINE_3+PAGE_FILES_LINE_HEIGHT-1, (currentItem - firstDisplayedItem == 2) ? 3:0);
            LCDD_PutString(PAGE_LINE_INDENT, PAGE_FILES_LINE_3, 0, strings[STRINGS_INDEX_CORRIDORHALLAUTO+STRINGS_TOTAL_LANGUAGES*(firstDisplayedItem+2)+language], (currentItem - firstDisplayedItem == 2) ? 3:0, 0);
        }

        if (PAGE_DEFAULTFILES_TOTALITEMS >= 4)
        {
            LCDD_ClearRect(0, PAGE_FILES_LINE_4, LCD_X_LENGTH-1, PAGE_FILES_LINE_4+PAGE_FILES_LINE_HEIGHT-1, (currentItem - firstDisplayedItem == 3) ? 3:0);
            LCDD_PutString(PAGE_LINE_INDENT, PAGE_FILES_LINE_4, 0, strings[STRINGS_INDEX_CORRIDORHALLAUTO+STRINGS_TOTAL_LANGUAGES*(firstDisplayedItem+3)+language], (currentItem - firstDisplayedItem == 3) ? 3:0, 0);
        }
...
#define PAGE_SENDFILE_PGBAR_STEP     (PAGE_SENDFILE_PGBAR_LENGTH/PARAMS_WRITABLE_TOTAL)
    ...
    if (pageInitialize)
    {
        ...
        yOffset = PAGE_SENDFILE_BODY_Y_START+LCDD_PutString(PAGE_SENDFILE_BODY_X_START, PAGE_SENDFILE_BODY_Y_START, PAGE_SENDFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_WAITSENDING+language], 3, 1)+PAGE_SENDFILE_BODY_INDENT;
        ...
        LCDD_PutString(PAGE_SENDFILE_BODY_X_START, PAGE_SENDFILE_BODY_Y_START, PAGE_SENDFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_WAITSENDING+language], 3, 0);
    ...
    if (pageUpdate)
    {
        if (currentParameter < PARAMS_WRITABLE_TOTAL )
        {
            ...
            {
              if(currentParameter<36 || currentParameter>56)
              {
        ...
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F2)
        {
            ...
            while (retry < PAGE_PARAM_READ_RETRY)
            {
                ...
                if (result == 1)    // reading successfully
                {
                    ...
                    {
                        string = strings[STRINGS_INDEX_DEFAULTFILESNA+language];
                        break;
                    }
                }
                else if (result == 2)
...
#define PAGE_DELETEFILE_BODY_Y_START   (PAGE_DELETEFILE_RECT_Y_START+PAGE_DELETEFILE_BODY_INDENT)
    ...
    if (pageInitialize)
    {
        ...
        yEnd = PAGE_DELETEFILE_BODY_Y_START+LCDD_PutString(PAGE_DELETEFILE_BODY_X_START, PAGE_DELETEFILE_BODY_Y_START, PAGE_DELETEFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_DELETEFILE+language], 3, 1);
        ...
        LCDD_PutString(PAGE_DELETEFILE_BODY_X_START, PAGE_DELETEFILE_BODY_Y_START, PAGE_DELETEFILE_BODY_X_LENGTH, strings[STRINGS_INDEX_DELETEFILE+language], 3, 0);
...
#define PAGE_PNLENTRY_CNTDOWN_ERASE     10  // 10 seconds
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_PNL+language]), 0, 0, strings[STRINGS_INDEX_PNL+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_PNLENTRY_LINE_1, 0, strings[STRINGS_INDEX_PNLGROUP+language], (currentItem == 0)?3:0, 0);

        LCDD_ClearRect(0, PAGE_PNLENTRY_LINE_2, LCD_X_LENGTH-1, PAGE_PNLENTRY_LINE_2+PAGE_PNLENTRY_LINE_HEIGHT-1, (currentItem == 1)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_PNLENTRY_LINE_2, 0, strings[STRINGS_INDEX_ADDPERIPHERAL+language], (currentItem == 1)?3:0, 0);

        memset(pageFunckeyItems, 0, sizeof(pageFunckeyItems));
        pageFunckeyItems[0].pItem = (void *)&BitmapInfo_ArrowReturn;
        if (currentItem == 0)   // redraw the function key area for "PnL Group"
        {
            pageFunckeyItems[2].pItem = (void *)strings[STRINGS_INDEX_ERASE+language];
            pageFunckeyItems[2].itemType = 1;
            pageFunckeyItems[3].pItem = (void *)strings[STRINGS_INDEX_LEARN+language];      // 2.4.0
...
//            BUTTON_Acknowledge();
        ...
        {
            LCDD_ClearRect(pageFunckeyRect[2].RectStartX, PAGE_FUNCKEY_Y_START, pageFunckeyRect[2].RectEndX, PAGE_FUNCKEY_Y_END, 0);
            _put_funckey_string(pageFunckeyRect[2].RectStartX, pageFunckeyRect[2].RectEndX, strings[STRINGS_INDEX_ERASE+language]);
        }
        else if ((BUTTON_GetButtonIndex() == BUTTON_INDEX_F3) && (currentItem == 1))
        {
            LCDD_InvertRect(pageFunckeyRect[2].RectStartX, PAGE_FUNCKEY_Y_START, pageFunckeyRect[2].RectEndX, PAGE_FUNCKEY_Y_END, PAGE_HIGHLIGHT_COLOR);
    ...
    else if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PRESSED) && (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3) && (currentItem == 0) && (BUTTON_GetPressedTime()%1000 == 0) && pageButtonActive)
    {
        ...
        else
        {
            ...
            for (i=0; i<16; i++)
            {
                LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_DELETEGROUP+language]), PAGE_PNLENTRY_BODY_Y_POS, 0, strings[STRINGS_INDEX_DELETEGROUP+language], 0, 0);
                LCDD_Refresh();
                TIME_Wait(52);
...
#define PAGE_PNL_CNTDOWN_INCLUDE        2   // 2 seconds
    ...
    if (pageInitialize)
    {
        ...
        memset(pageFunckeyItems, 0, sizeof(pageFunckeyItems));
        pageFunckeyItems[0].pItem = (void *)strings[STRINGS_INDEX_END+language];
    ...
    if (pageUpdate)
    {
        ...
        {
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_LEARN+language]), 0, 0, strings[STRINGS_INDEX_LEARN+language], 0, 0);
        }
...
#define PAGE_SETTINGS_LINE_4         (PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT+PAGE_SETTINGS_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_SETTINGS+language]), 0, 0, strings[STRINGS_INDEX_SETTINGS+language], 0, 0);
...
//        currentItem = 0;
    ...
    if (pageUpdate)
    {
        ...
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_1, 0, strings[STRINGS_INDEX_LANGUAGES+language], (currentItem == 0)?3:0, 0);
        ...
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0, strings[STRINGS_INDEX_VERSION+language], (currentItem == 1)?3:0, 0);
        ...
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_3, 0, strings[STRINGS_INDEX_BATTERY+language], (currentItem == 2)?3:0, 0);
        ...
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4, 0, strings[STRINGS_INDEX_APPSWITCH+language], (currentItem == 3)?3:0, 0);
        LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_4, (currentItem == 3)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
    #endif  // #ifndef NO_APPSWITCH
#else
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_1, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_1+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 0)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_1, 0, strings[STRINGS_INDEX_VERSION+language], (currentItem == 0)?3:0, 0);
        LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_1, (currentItem == 0)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);

        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_2, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_2+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 1)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0, strings[STRINGS_INDEX_BATTERY+language], (currentItem == 1)?3:0, 0);
        LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_2, (currentItem == 1)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);

    #ifndef NO_APPSWITCH
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_3, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 2)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_3, 0, strings[STRINGS_INDEX_APPSWITCH+language], (currentItem == 2)?3:0, 0);
        LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_3, (currentItem == 2)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
    #endif  // #ifndef NO_APPSWITCH
#endif  // #ifndef APP_BAES
   ...
   // static const unsigned char ver[] = {'V','e','r',':',' ',VERSION_NUMBER,'\0'};
    ...
    if (pageInitialize)
    {
        ...
        if (language == Unknown)
        {
            ...
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, VERSION_NUMBER), 0, 0, VERSION_NUMBER, 0, 0);
        }
        else
        {
            pageParent = PAGE_INDEX_SETTINGS;
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_LANGUAGES+language]), 0, 0, strings[STRINGS_INDEX_LANGUAGES+language], 0, 0);
        }
...
#define PAGE_VERSION_BODY_Y_POS     20
    ...
    static const unsigned char version[] = {"Firmware:"};
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(PAGE_LINE_INDENT+60, PAGE_VERSION_BODY_Y_POS, 0,VERSION_NUMBER, 0, 0);
    }
...
#define PAGE_BATTERY_EXCLAMATION_Y_POS      (((LCD_Y_LENGTH-PAGE_BATTERY_TOP_SECTION_HEIGHT-PAGE_FUNCKEY_AREA_HEIGHT-PAGE_BATTERY_EXCLAMATION_HEIGHT)/2+PAGE_BATTERY_TOP_SECTION_HEIGHT)/4*4)
    ...
    if (pageUpdate)
    {
        ...
        {
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_LOWBATTERY+language]), 0, 0, strings[STRINGS_INDEX_LOWBATTERY+language], 0, 0);
        ...
        {
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_BATTERYLEVEL+language]), 0, 0, strings[STRINGS_INDEX_BATTERYLEVEL+language], 0, 0);
...
#define PAGE_APPSWITCH_BODY_Y_START     (PAGE_APPSWITCH_RECT_Y_START+PAGE_APPSWITCH_BODY_INDENT)
    ...
    if (pageInitialize)
    {
        ...
        yOffset = PAGE_APPSWITCH_BODY_Y_START+LCDD_PutString(PAGE_APPSWITCH_BODY_X_START, PAGE_APPSWITCH_BODY_Y_START, PAGE_APPSWITCH_BODY_X_LENGTH, strings[STRINGS_INDEX_APPSWITCHPROMPT+language], 3, 1)+PAGE_APPSWITCH_BODY_INDENT;
        ...
        {
            appStatus = 1;
            LCDD_PutString(PAGE_APPSWITCH_BODY_X_START, PAGE_APPSWITCH_BODY_Y_START, PAGE_APPSWITCH_BODY_X_LENGTH, strings[STRINGS_INDEX_APPSWITCHPROMPT+language], 3, 0);
            pageFunckeyItems[1].pItem = (void *)strings[STRINGS_INDEX_YES+language];
            pageFunckeyItems[1].itemType = 1;
            pageFunckeyItems[2].pItem = (void *)strings[STRINGS_INDEX_NO_1+language];
            pageFunckeyItems[2].itemType = 1;
        ...
        {
            appStatus = 0;
            LCDD_PutString(PAGE_APPSWITCH_BODY_X_START, PAGE_APPSWITCH_BODY_Y_START, PAGE_APPSWITCH_BODY_X_LENGTH, strings[STRINGS_INDEX_NOAPPFOUND+language], 3, 0);
            pageFunckeyItems[0].pItem = (void *)&BitmapInfo_ArrowReturn;
        }
        _common_funckey_initializer();
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED))
    {
        ...
        else if ((BUTTON_GetButtonIndex() == BUTTON_INDEX_F2) && (appStatus == 1))
        {
            ...
            LCDD_ClearRect(PAGE_APPSWITCH_RECT_X_START, PAGE_APPSWITCH_RECT_Y_START, PAGE_APPSWITCH_RECT_X_END, yOffset, 3);
            LCDD_PutString(PAGE_APPSWITCH_BODY_X_START, PAGE_APPSWITCH_BODY_Y_START, PAGE_APPSWITCH_BODY_X_LENGTH, strings[STRINGS_INDEX_RESTART+language], 3, 0);
            LCDD_Refresh();
            TIME_Wait(2000);
...
//        _param_page_funckey_initializer(FUNCKEY_WRITE_ONLY);      // 2.3.0
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_TEST+language]), 0, 0, strings[STRINGS_INDEX_TEST+language], 0, 0);     // 2.4.0

//        currentItem = 0;      // 2.3.0
        if(sensorParams.Type.Value==0x112)
        {
          pStrings[0] = strings[STRINGS_INDEX_ENDTIMEDELAY+language];
          pStrings[1] = strings[STRINGS_INDEX_INITIALSTATE+language];
        ...
        else
        {
          ...
          pStrings[0] = strings[STRINGS_INDEX_WALKTEST+language];
          pStrings[1] = strings[STRINGS_INDEX_ENDTIMEDELAY+language];
          pStrings[2] = strings[STRINGS_INDEX_INITIALSTATE+language];
        }
        // 2.3.0
        if (PARAMS_IsZigbeeSensor(&sensorParams))
        {
            pStrings[3] = strings[STRINGS_INDEX_CONTROL+language];
            totalItems = 4;
        }
        else if(sensorParams.Type.Value==0x112)
        {
        ...
        {
            pStrings[3] = strings[STRINGS_INDEX_ON+language];       // 2.4.0
            pStrings[4] = strings[STRINGS_INDEX_OFF+language];      // 2.4.0
            totalItems = 5;
        }
...
#define PAGE_CONTROL_LINE_4                (PAGE_CONTROL_LINE_3+PAGE_CONTROL_LINE_HEIGHT+PAGE_CONTROL_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        memset(pageFunckeyItems, 0, sizeof(pageFunckeyItems));
        pageFunckeyItems[0].pItem = (void *)&BitmapInfo_ArrowReturn;
        pageFunckeyItems[2].pItem = (void *)strings[STRINGS_INDEX_ON+language];     // 2.4.0
        pageFunckeyItems[2].itemType = 1;
        pageFunckeyItems[3].pItem = (void *)strings[STRINGS_INDEX_OFF+language];    // 2.4.0
        pageFunckeyItems[3].itemType = 1;
        _common_funckey_initializer();

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_CONTROL+language]), 0, 0, strings[STRINGS_INDEX_CONTROL+language], 0, 0);
    ...
    if (pageUpdate)
    {
        ...
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_CONTROL_LINE_1, 0, strings[STRINGS_INDEX_LIGHTS+language], (currentItem == 0)?3:0, 0);

        LCDD_ClearRect(0, PAGE_CONTROL_LINE_2, LCD_X_LENGTH-1, PAGE_CONTROL_LINE_2+PAGE_CONTROL_LINE_HEIGHT-1, (currentItem == 1)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_CONTROL_LINE_2, 0, strings[STRINGS_INDEX_AUXILIARIES+language], (currentItem == 1)?3:0, 0);
    }
...
#define PAGE_HELP_BODY_Y_POS        20
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_HELP_BODY_Y_POS, LCD_X_LENGTH-2, strings[STRINGS_INDEX_HELP+language], 0, 0);
    }
...
#define PAGE_ROOMCONTROL_LINE_4                (PAGE_ROOMCONTROL_LINE_3+PAGE_ROOMCONTROL_LINE_HEIGHT+PAGE_ROOMCONTROL_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings[STRINGS_INDEX_ROOMCONTROL+language]), 0, 0, strings[STRINGS_INDEX_ROOMCONTROL+language], 0, 0);

        currentItem = 0;

        pStrings[0] = strings[STRINGS_INDEX_REINITIALIZEROOM+language];
        pStrings[1] = strings[STRINGS_INDEX_RETURNTOFACTORY+language];
...
#define PAGE_SETTINGS_LINE_4         (PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT+PAGE_SETTINGS_LINE_SPACE)
    ...
    {
      pageInitialize = 0;
      pParameter = (PAGE_PARAMETER *)malloc(1*sizeof(PAGE_PARAMETER));    // 2.3.1, was 3
      pParameter->ParamIndex = PARAM_INDEX_KNX_MASTER_SLAVE;
      LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);
      _param_page_funckey_initializer(FUNCKEY_NONE);
      LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, KNX_sensor_master_langeage_string[language]), 0, 0, KNX_sensor_master_langeage_string[language], 0, 0);
...
#ifndef APP_BAES
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_1, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_1+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 0)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_1, 0, DALI_ballast_name[language], (currentItem == 0)?3:0, 0);
        LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_1, (currentItem == 0)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);

        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_2, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_2+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 1)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0, DALI_sensor_name[language], (currentItem == 1)?3:0, 0);
        LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_2, (currentItem == 1)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);

        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_3, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 2)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_3, 0,DALI_changeToSlave_name[language], (currentItem == 2)?3:0, 0);
        //LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_3, (currentItem == 2)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
        
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 3)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4, 0, DALI_eraseDaliSys_name[language], (currentItem == 3)?3:0, 0);
       // LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_3, (currentItem == 2)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED))
    {
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_DOWN && flag==0)
        {
            if (currentItem == PAGE_SETTINGS_MAXIMUMITEMS-1)
            {
                currentItem = 0;
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_UP && flag==0)
        {
            if (currentItem == 0)
            {
                currentItem = PAGE_SETTINGS_MAXIMUMITEMS-1;
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_RIGHT && flag==0)
        {
          if(currentItem<=1)
          { 
            pageCurrent = targetPage[currentItem];
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
        {
          ...
          {   
            ...
            {
              {          
                WaringBlock(DALI_warning_name[language],DALI_warning_long_name[language]);
                flag ++ ;
              }  
            }
            else
          ...
          if(currentItem==3)
          {
            ...
            {
              {          
                WaringBlock(DALI_warning_name[language],DALI_erase_long_name[language]);
                flag ++ ;
              }  
            }
            else
            {
              ...
              TIME_Wait(10);
              IR_PnLGroup(NITOO_LM058A_NODE, CODE_START_GROUP_PNL, 0x4A);
              flag --;
                              pageSwap =  1;
            }
...
#define PAGE_SENSITIVITY_BODY_Y_POS_3       (PAGE_SENSITIVITY_BODY_Y_POS_2 + PAGE_SENSITIVITY_BODY_Y_POS_DELTA)
  ...
  // static unsigned char previousItem;
  static unsigned char currentItem;
  static unsigned char totalItems=5;
  static PAGE_PARAMETER *pParameter;
  unsigned char yOffset, i;
...
//          GroupNum.GroupTmp[i]=0;
      ...
      //  LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1,0, 0, "Choose Zone", 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_salve_name[language]), 0, 0, DALI_salve_name[language], 0, 0); 
    ...
    if (pageUpdate)
    {
      ...
      LCDD_ClearRect(0, PAGE_SETTINGS_LINE_2, 128, PAGE_SETTINGS_LINE_2+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 1)?3:0);
      //LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, PAGE_SETTINGS_LINE_2, 0, DALI_viewGroup_name[language], (currentItem == 1)?3:0, 0);
      // LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_1, (currentItem == 0)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
      
      if(1==currentItem)
      {  if (LCDD_PutScrollString(PAGE_SENSITIVITY_NAME_X_POS, PAGE_SETTINGS_LINE_2, 124, DALI_viewGroup_name[language]+pageScroll, ((1 == currentItem)?PAGE_HIGHLIGHT_COLOR:0), (currentItem == 1)?3:0))
      {
        pageScroll++;
        pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
      }
        ...
        LCDD_PutScrollString(PAGE_SENSITIVITY_NAME_X_POS, PAGE_SETTINGS_LINE_2, 124, DALI_viewGroup_name[language], ((1 == currentItem)?PAGE_HIGHLIGHT_COLOR:0), 0);
      
      LCDD_ClearRect(0, PAGE_SETTINGS_LINE_3-5, 128, PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 2)?3:0);
      //LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, PAGE_SETTINGS_LINE_3-5, 0,DALI_exfromthisgroup_name[language], (currentItem == 2)?3:0, 0);               
      if(2==currentItem)  
      { if (LCDD_PutScrollString(PAGE_SENSITIVITY_NAME_X_POS,  PAGE_SETTINGS_LINE_3-5, 124, DALI_exfromthisgroup_name[language]+pageScroll, ((2 == currentItem)?PAGE_HIGHLIGHT_COLOR:0), (currentItem == 2)?3:0))
      {
        pageScroll++;
        pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
      }
        ...
        LCDD_PutScrollString(PAGE_SENSITIVITY_NAME_X_POS,  PAGE_SETTINGS_LINE_3-5, 124, DALI_exfromthisgroup_name[language], ((2 == currentItem)?PAGE_HIGHLIGHT_COLOR:0), 0);
      
      LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4-5-5-1, 128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 3)?3:0);
      //LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, PAGE_SETTINGS_LINE_4-5-5-1, 0, DALI_ExAllGroup_name[language], (currentItem == 3)?3:0, 0); 
      if(3==currentItem)
      {   
        if (LCDD_PutScrollString(PAGE_SENSITIVITY_NAME_X_POS,PAGE_SETTINGS_LINE_4-5-5-1, 124, DALI_ExAllGroup_name[language]+pageScroll, ((3 == currentItem)?PAGE_HIGHLIGHT_COLOR:0), (currentItem == 3)?3:0))
        {
          pageScroll++;
          pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
        }    
        ...
        LCDD_PutScrollString(PAGE_SENSITIVITY_NAME_X_POS,PAGE_SETTINGS_LINE_4-5-5-1, 124, DALI_ExAllGroup_name[language], ((3 == currentItem)?PAGE_HIGHLIGHT_COLOR:0), 0);
      LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4+3+2, 128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT+2+2, (currentItem == 4)?3:0);
      if(4==currentItem)      
      { //LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, PAGE_SETTINGS_LINE_4+3+2, 0, DALI_tomaster_name[language], (currentItem == 4)?3:0, 0);  
        if (LCDD_PutScrollString(PAGE_SENSITIVITY_NAME_X_POS,PAGE_SETTINGS_LINE_4+3+2, 124, DALI_tomaster_name[language]+pageScroll, ((4 == currentItem)?PAGE_HIGHLIGHT_COLOR:0), (currentItem == 4)?3:0))
        {
          pageScroll++;
          pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
        }
        ...
        LCDD_PutScrollString(PAGE_SENSITIVITY_NAME_X_POS,PAGE_SETTINGS_LINE_4+3+2, 124, DALI_tomaster_name[language], ((4 == currentItem)?PAGE_HIGHLIGHT_COLOR:0), 0);
      
      if (currentItem == 0)
      {
        LCDD_ClearRect(PAGE_SENSITIVITY_NAME_X_POS, rect[currentItem].yStart, LCD_X_LENGTH-1, rect[currentItem].yEnd, 0);
        LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS-3, yOffset, 0, DALI_group_name[language], 0, 0);
        LCDD_InvertRect(PAGE_SENSITIVITY_NAME_X_POS-3, rect[currentItem].yStart, LCD_X_LENGTH-1, rect[currentItem].yEnd, PAGE_HIGHLIGHT_COLOR);           
      }
      else 
      {
        LCDD_ClearRect(PAGE_SENSITIVITY_NAME_X_POS-3, rect[0].yStart, LCD_X_LENGTH-1, rect[0].yEnd, 0);
        LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS-3, 18, 0, DALI_group_name[language], 0, 0);
      }
      ...
      for (i=0; i<1; i++)
      {
        ...
        { 
          ...
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS-3, 18, 0,DALI_group_name[language], 0, 0); 
        ...
        //  LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS+4, PAGE_SETTINGS_LINE_4+3, 0, ">", 0, 0);   
      }
...
//          }
        ...
        else if ((BUTTON_GetButtonIndex() == BUTTON_INDEX_F3))
        {
          if(currentItem==1)            
          {//indentify
              IR_ReadNBofControlGearParameter(NITOO_LM058A_NODE,0xB0,0x48,60,GroupNum.GroupTmp[currentItem_x]);
          }
          if(currentItem==2)            
          {
...
//            }
            ...
            IR_SetGroupNum(NITOO_LM058A_NODE,0xF0,0x4D,DataGroup.DATA2, DataGroup.DATA3, DataGroup.DATA4,DataGroup.DATA5,0);            
           
            //TIME_Wait(25);
            if(_page_read_GroupNumAll()==10)
            {
          ...
          if(currentItem==4)
          {
            pageTimeStamp = 0xffffffff;
              if(flag==0)
              {
                {          
                  WaringBlock(DALI_waring_name[language],DALI_warning_long_name[language]);
                  flag ++ ;
                }  
              }
              else
              {
                ...
                LCDD_ClearRect(PAGE_READPARAMS_RECT_X_START-5, PAGE_READPARAMS_RECT_Y_START, PAGE_READPARAMS_RECT_X_END+5, 90+20, 3);
                _page_sub_read_parameters(pParameter, 1); 
                currentItem = PARAMS_GetParameterValue(&sensorParams,PARAM_INDEX_KNX_MASTER_SLAVE);
                if(currentItem==0)
                { 
...
#define PAGE_SETTINGS_LINE_4         (PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT+PAGE_SETTINGS_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);
        _param_page_funckey_initializer(FUNCKEY_NONE);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_ballast_name[language]), 0, 0, DALI_ballast_name[language], 0, 0);
...
#ifndef APP_BAES
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_1, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_1+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 0)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_1, 0, DALI_createAssociation_name[language], (currentItem == 0)?3:0, 0);
        LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_1, (currentItem == 0)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);

        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_2, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_2+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 1)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0, DALI_viewAssociation_name[language], (currentItem == 1)?3:0, 0);
       // LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_2, (currentItem == 1)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
...
//              else 
        ...
        {
            pageCurrent = PAGE_INDEX_DALI_MASTER;
          ...
          {           
            ...
            {   
              ...
              for(i=1;i<4;i++)
              {
                ...
                if( result_read!=2)
                {
                  if(result_read==0)
                  {
                    WaringBlock("CRC","ERROR"); 
                    TIME_Wait(2000);
                    pageSwap =  1;
                  }
                  if(result_read==3)
                  {
                    WaringBlock("FRAME","ERROR"); 
                    TIME_Wait(2000);
                    pageSwap =  1;
                  }
                  break;
              ...
              if(result_read==2)
              {
                ...
                { 
                  if(i==3) continue;
                  IR_ReadNBofControlGearParameter(NITOO_LM058A_NODE,CODE_OPEN_LEARNING,0x41,i,0x00);
                  TIME_Wait(2);
                  result_read= IR_Read_CONTROL_GEAR_Info(i);
                  if( result_read!=2)
                  {
                    if(result_read==0)
                    {
                      WaringBlock("CRC 2","ERROR"); 
                      TIME_Wait(2000);
                      pageSwap =  1;
                    }
                    if(result_read==3)
                    {
                      WaringBlock("FRAME 2","ERROR"); 
                      TIME_Wait(2000);
                      pageSwap =  1;
                    }
                    break;
                ...
                if(result_read==2)
                {
                  ...
                  pageCurrent= PAGE_VIEW_AREA ;//view page
                  pageSwap=1;
                }
              }  
            }
            else if((sensorParams.Type.Value==PARAM_TYPE_DALI_MASTER_DL_OP_PIR) || (sensorParams.Type.Value==PARAM_TYPE_DALI_MASTER_OP_PIR)|| (sensorParams.Type.Value==PARAM_TYPR_DALI_MASTER_DL_PIR) )
            {
              ...
              if(result_read==2)
              {
                ...
                TIME_Wait(2);
                result_read= IR_Read_CONTROL_GEAR_Info(01);
                if( result_read==2)
                {
                  pageCurrent= PAGE_VIEW_AREA_CORRID ;//view page
                  pageSwap=1;
                }
                else
                {
                  WaringBlock("Gear info","ERROR"); 
                  TIME_Wait(2000);
                  pageSwap =  1;
                }
              } 
...
#define PAGE_SETTINGS_LINE_4         (PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT+PAGE_SETTINGS_LINE_SPACE)
  ...
  static unsigned char currentItem = 0;
  static unsigned char currentItemX = 0;
  static unsigned char previousItem;
  static const PAGE_ITEM_RECT rect[3] = {{PAGE_SENSITIVITY_BODY_Y_POS_1-1, PAGE_SENSITIVITY_BODY_Y_POS_1+16}, {PAGE_SENSITIVITY_BODY_Y_POS_2-1-1-1, PAGE_SENSITIVITY_BODY_Y_POS_2+16-1-1}, {PAGE_SENSITIVITY_BODY_Y_POS_3-1, PAGE_SENSITIVITY_BODY_Y_POS_3+16}};
#ifndef APP_BAES
  static unsigned char enterHiddenPage;
  static unsigned int timeStamp;
#endif  // #ifndef APP_BAES
    ...
    if (pageInitialize)
    {
        ...
        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_sensor_name[language]), 0, 0, DALI_sensor_name[language], 0, 0);
       _param_pageDali_funckey_initializer(FUNCKEY_WRITE_ONLY);
//        currentItem = 0;
    ...
    if (pageUpdate)
    {
        ...
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_2-1, LCD_X_LENGTH, PAGE_SETTINGS_LINE_2+PAGE_SETTINGS_LINE_HEIGHT-1+1, 0);

        //LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0, DALI_viewslave_name[language], 0, 0);
        LCDD_PutScrollString(0, PAGE_SETTINGS_LINE_2, 70-5, DALI_viewslave_name[language], 0, 1);
        if(currentItem==1)
        {  
          if (LCDD_PutScrollString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 70-5-3, DALI_viewslave_name[language]+pageScroll, 0, 1))
          {
            pageScroll++;
            pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
          }
          ...
          LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4+10+5+3+8, PAGE_SETTINGS_LINE_2, LCD_X_LENGTH-2, PAGE_SETTINGS_LINE_2+15, 0);
        }  //  LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS, yOffset, 0, strings[STRINGS_INDEX_LOW+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, (pParameter+i)->ParamIndex)+language], 0, 0);      // 2.4.0
          LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, PAGE_SETTINGS_LINE_2, 0, "<>", 0, 0);
        if(currentItemX==0)
        {
          LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS-50, PAGE_SETTINGS_LINE_2, 0, DALI_start2_name[language], 0, 0);
        }
        else
        {
          LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS-50, PAGE_SETTINGS_LINE_2, 0, DALI_stop_name[language], 0, 0);
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED))
    {
        ...
        {
           if(currentItem==1 && currentItemX ==0)
           {
             IR_ReadNBofControlGearParameter(NITOO_LM058A_NODE,0xB0,0x47,0x3c,0x00);      
           }
            if(currentItem==1 && currentItemX ==1)
           {
  ...
  //  static unsigned char enterHiddenPage;
    ...
    if (pageInitialize)
    {
       ...
       // LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, "Master"), 0, 0, "Master", 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_createAssociation_name[language] ), PAGE_SETTINGS_LINE_1, 0, DALI_createAssociation_name[language] , 0, 0);
...
//        currentItem = 0;
    ...
    if (pageUpdate)
    {
        ...
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0, DALI_start_name[language], (currentItem == 0)?3:0, 0);
       // LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_1, (currentItem == 0)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
        ...
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_3, 0,DALI_exit_name[language], (currentItem == 1)?3:0, 0);
       // LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_2, (currentItem == 1)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
    ...
    #endif  // #ifndef NO_APPSWITCH
#endif  // #ifndef APP_BAES
               LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4-5-5+2, 128,PAGE_SETTINGS_LINE_4+5+10+1+2+2, 0); 
        if(currentItem==1)
        { 
          LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-5+2, 0, DALI_stopProcedure_name[language], 0, 0);
          LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4+5+2, 0, DALI_pressF3_name[language], 0, 0);
}
                  _param_page_funckey_initializer(FUNCKEY_READ_WRITE);
        if(currentItem==0&&beginShow==1)
          _param_page_funckey_initializer(FUNCKEY_READ_WRITE);
...
timeStamp = TIME_GetTime();
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)//read
        {
          if(currentItem==0)
          {
//            LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4, 128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT-1, 0);
//            LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-5, 0, DALI_wait_name[], 0, 0);
//            LCDD_PutString(0, PAGE_SETTINGS_LINE_4+5, 0, DALI_pressF2_name[], 0, 0);
            
//            LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4, 128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT-1, 0);
//            LCDD_Refresh();
//            LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-5, 0, DALI_wait_name[language], 0, 0);
//            LCDD_PutString(0, PAGE_SETTINGS_LINE_4+5, 0, DALI_pressF2_name[language], 0, 0); 
//            timeStamp = TIME_GetTime();
                   timeStamp = TIME_GetTime();
          ...
          if(currentItem==1)
          {
//            LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4, 128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT-1+2, 0);
//            LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-5, 0, DALI_stopProcedure_name[language], 0, 0);
//            LCDD_PutString(0, PAGE_SETTINGS_LINE_4+5, 0, DALI_pressF3_name[language], 0, 0);
...
//    if(TIME_GetTime()>(timeStamp +1500) && currentItem==0 && timeStamp >0)
//    {
//      LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4, 128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT+2, 0);
//      LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-5, 0,  DALI_wait_name1[language], 0, 0);
//      LCDD_PutString(0, PAGE_SETTINGS_LINE_4+5, 0, DALI_pressF2_name1[language], 0, 0);
//      timeStamp=0;
//    }
    ...
    {  
     ...
     // timeStampTemp = timeStamp;
      if(TIME_GetTime()==(timeStamp+500))
      {
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4-5-1, 128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT+5-1, 0);
        LCDD_Refresh();
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-5, 0, DALI_wait_name[language], 0, 0);
        LCDD_PutString(0, PAGE_SETTINGS_LINE_4+5, 0, DALI_pressF2_name[language], 0, 0); 
        //pageUpdate = 1;
      }
      if(TIME_GetTime()==(timeStamp +2500))
      {
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4, 128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT+2, 0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-5, 0,  DALI_wait_name1[language], 0, 0);
        LCDD_PutString(0, PAGE_SETTINGS_LINE_4+5, 0, DALI_pressF2_name1[language], 0, 0);
      ...
      if(TIME_GetTime()==(timeStamp +3000))
      {
        timeStamp=TIME_GetTime();
...
//        currentItem = 0;
    ...
    if (pageUpdate)
    {
        ...
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_2, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_1+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 0)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0, DALI_nogroup_name[language],0, 0);
       // LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_1, (currentItem == 0)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
 ...
 //  static struct DATAGroup test[32];
    ...
    if (pageInitialize)
    {
       ...
       // _param_page_funckey_initializer(FUNCKEY_READ_WRITE);
        ...
        _param_page_funckey_initializer(FUNCKEY_WRITE_ONLY);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_addslave_name[language]), 0, 0, DALI_addslave_name[language], 0, 0);
       // LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, "Create Group"), PAGE_SETTINGS_LINE_1, 0, "Create Group", 0, 0);        
        
        LCDD_PutString(10, PAGE_SETTINGS_LINE_1, 0, DALI_group_name[language], 0, 0);
    ...
    if (pageUpdate)
    {
      ...
      //  LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0, "start", (currentItem == 0)?3:0, 0);
       // LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_1, (currentItem == 0)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
        ...
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_2, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_2+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 0)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0, DALI_include_name[language], (currentItem == 0)?3:0, 0);
  
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_3, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 1)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_3, 0, DALI_exclude_name[language], (currentItem == 1)?3:0, 0);

        
         dectoascii(&pSec[0],zone_value, 0);
         LCDD_PutString(100, PAGE_SETTINGS_LINE_1, 0, &pSec[0],PAGE_HIGHLIGHT_COLOR, 0);   
         
        
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4-5,128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT-1+5, 0);
        
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-3, 0, DALI_aimsensor_name[language],0, 0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4+5+2, 0, DALI_aimsensor2_name[language],0, 0);
//        if(LCDD_PutScrollString(0, PAGE_SETTINGS_LINE_4, 128-2, " Aim at detector and press:"+pageScroll,0, 1))
//        {
//          pageScroll++;
//          pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY_DALI;
//        }
//         
        ...
        if(BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
        {
         // zone_value=0x10;
          ...
          {//include group           
            ...
            IR_SetGroupNum(NITOO_LM058A_NODE,0xF0,0x46,DataGroup.DATA2, DataGroup.DATA3, DataGroup.DATA4,DataGroup.DATA5,detec_short_addres);
              //TIME_Wait(25);
            //IR_SetGroupNum(NITOO_LM058A_NODE,0xF0,0xb1,0x04, 0xff, 0xff,0,0);
            
            //IR_ReadSensorParameter(&sensorParams, PARAM_INDEX_OFFset);
...
static PAGE_PARAMETER * pParameter;
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED))
    {
      if (BUTTON_GetButtonIndex() == BUTTON_INDEX_DOWN && flag==0)
      {
        if (currentItem == PAGE_SETTINGS_MAXIMUMITEMS-2)
        {
          currentItem = 0;
        }
        else
      ...
      else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_UP && flag==0)
      {
        if (currentItem == 0)
        {
          currentItem = PAGE_SETTINGS_MAXIMUMITEMS-2;
        }
        else
        {
          currentItem--;
      ...
      {
        //PARAMS_SetParameterNumber(&sensorParams, PARAM_INDEX_TYPE, PARAM_NUMBER_TYPE2);
        //        pParameter = (PAGE_PARAMETER *)malloc(sizeof(PAGE_PARAMETER));
        //        pParameter->ParamIndex = PARAM_INDEX_KNX_MASTER_SLAVE;    
        if(flag==0)
        {
      ...
      else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
      {
        ...
        {
          //slave to master
          //          if(sensorParams.KNX_Master_slave.Value==1)
          //          {            
...
void WaringBlock(unsigned char const *string_1, unsigned char const *string_2)
{
  ...
  LCDD_ClearRect(PAGE_READPARAMS_RECT_X_START-5, PAGE_READPARAMS_RECT_Y_START, PAGE_READPARAMS_RECT_X_END+5, 90+20, 3);
  LCDD_PutString(PAGE_READPARAMS_BODY_X_START-5, PAGE_READPARAMS_BODY_Y_START, PAGE_READPARAMS_BODY_X_LENGTH+5, string_1, 3, 0);
  LCDD_PutString(PAGE_READPARAMS_BODY_X_START-5, PAGE_READPARAMS_BODY_Y_START+20, PAGE_READPARAMS_BODY_X_LENGTH+15, string_2, 3, 0);
  LCDD_Refresh();     // Have to do this here.
...
#define PAGE_SETTINGS_LINE_4         (PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT+PAGE_SETTINGS_LINE_SPACE)
   ...
   static unsigned int timeStamp;
    ...
    if (pageInitialize)
    {
        ...
        _param_page_funckey_initializer(FUNCKEY_READ_WRITE);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1,DALI_creat_name[language]), 0, 0, DALI_creat_name[language], 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1,DALI_association_name[language]), PAGE_SETTINGS_LINE_1, 0, DALI_association_name[language], 0, 0);
...
//        currentItem = 0;
        ...
        //timeStamp = TIME_GetTime(); // Initial timeStamp to infinity
#endif  // #ifndef APP_BAES
    ...
    if (pageUpdate)
    {
       ...
       // LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_2, (currentItem == 1)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);
        ...
        if(currentItem==0)
        {
         ...
         // LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-5, 0, "Wait end of fast blink ", 0, 0);
         // LCDD_PutString(0, PAGE_SETTINGS_LINE_4+5, 0, "then press F2 to continue", 0, 0);
        }
...
//            currentItem=zone_value;
            int temp1;
            temp1 = _page_read_zone_info(FirstTime);
            if(temp1==2)
            {
              IR_PnLGroup(NITOO_LM058A_NODE, CODE_ERASE_GROUP_PNL, CMD_CANCEL_LEARNING);
              pageCurrent = PAGE_INDEX_MAIN;
     ...
     temp1=10;
            ...
            if(temp1==10)
            {
              ...
              TIME_Wait(3);
              _page_read_zone_info(SecondTime);
            }
//               pageCurrent = PAGE_CREATE_GROUP;
    ...
    {  
      // timeStamp = TIME_GetTime();
      // timeStampTemp = timeStamp;
      if(TIME_GetTime()==(timeStamp+1000))
      {
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4-5-1, 128, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT+5-1, 0);
        LCDD_Refresh();
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_4-5-5, 0, DALI_wait_name[language], 0, 0);
...
#define PAGE_SENSITIVITY_BODY_Y_POS_3       (PAGE_SENSITIVITY_BODY_Y_POS_2 + PAGE_SENSITIVITY_BODY_Y_POS_DELTA)
  ...
  const unsigned char *strings[]={DALI_include_name[language],DALI_exclude_name[language]}; 
  static unsigned char currentItem_x;
  const unsigned char *PString;
  unsigned char pMin[3];
  unsigned char pMan[3];
    ...
    if (pageInitialize)
    {
      ...
      //  LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1,0, 0, "Choose Zone", 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_CreateAssociation_name[language]), 0, 0, DALI_CreateAssociation_name[language], 0, 0); 
    ...
    if (pageUpdate)
    {
        ...
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_2, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_2+PAGE_SETTINGS_LINE_HEIGHT-1, (currentItem == 1)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_2, 0,  DALI_next_name[language], (currentItem == 1)?3:0, 0);
       // LCDD_PutBitmap(PAGE_ARROW_NAV_X_POS, PAGE_SETTINGS_LINE_1, (currentItem == 0)?&BitmapInfo_ArrowNav[1]:&BitmapInfo_ArrowNav[0], 0);

        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_3-5, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT-1-5, (currentItem == 2)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_3-5, 0, DALI_previours_name[language], (currentItem == 2)?3:0, 0);               
        
        
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_3+5+3+2, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_3+PAGE_SETTINGS_LINE_HEIGHT-1+5+2+3, (currentItem == 3)?3:0);
        LCDD_PutString(PAGE_LINE_INDENT, PAGE_SETTINGS_LINE_3+5+3+2, 0, DALI_exit_name[language], (currentItem == 3)?3:0, 0);  
        
        
        
        LCDD_ClearRect(0, PAGE_SETTINGS_LINE_4+5+3, PAGE_ARROW_NAV_X_POS-1, PAGE_SETTINGS_LINE_4+PAGE_SETTINGS_LINE_HEIGHT-1+5+3, 0);
        // LCDD_PutString(20-5, PAGE_SETTINGS_LINE_4+5+3, 0, "    of        Ballasts", 0, 0);
//        if(language==English)
//          LCDD_PutString(20-5, PAGE_SETTINGS_LINE_4+5+3, 0, "    of        Ballasts", 0, 0);
//        else
//          LCDD_PutString(20-5-2, PAGE_SETTINGS_LINE_4+5+3, 0, DALI_dispN_name[language], 0, 0);
//        
//        
//        dectoascii(&pMin[0],xx_createzone, 0);
//        LCDD_PutString(5, PAGE_SETTINGS_LINE_4+5+3, 0, &pMin[0],PAGE_HIGHLIGHT_COLOR, 0);
//        
//        dectoascii(&pMin[0],yy_createzone, 0);
//        LCDD_PutString((language==English)?50-5-5:120, PAGE_SETTINGS_LINE_4+5+3, 0, &pMin[0],PAGE_HIGHLIGHT_COLOR, 0);  
        
        
        
        LCDD_PutString(0, PAGE_SETTINGS_LINE_4+5+3, 0, DALI_dispN_name[language], 0, 0);
        
        dectoascii(&pMin[0],xx_createzone, 0);       
        dectoascii(&pMan[0],yy_createzone, 0);
        if(language==French || language==English || language==Spanish)
        { 
          LCDD_PutString(40, PAGE_SETTINGS_LINE_4+5+3, 0, &pMin[0],PAGE_HIGHLIGHT_COLOR, 0);        
          LCDD_PutString(112-20, PAGE_SETTINGS_LINE_4+5+3, 0, &pMan[0],PAGE_HIGHLIGHT_COLOR, 0); 
        }
        if(language==Italian)
        {         LCDD_PutString(70, PAGE_SETTINGS_LINE_4+5+3, 0, &pMin[0],PAGE_HIGHLIGHT_COLOR, 0);
        //LCDD_PutString((language==English)?50-5-5:120, PAGE_SETTINGS_LINE_4+5+3, 0, &pMin[0],PAGE_HIGHLIGHT_COLOR, 0);      
        LCDD_PutString(112, PAGE_SETTINGS_LINE_4+5+3, 0, &pMan[0],PAGE_HIGHLIGHT_COLOR, 0); 
        }
        if(language==German)
        {         
          LCDD_PutString(70+10-3, PAGE_SETTINGS_LINE_4+5+3, 0, &pMin[0],PAGE_HIGHLIGHT_COLOR, 0);           
          LCDD_PutString(112, PAGE_SETTINGS_LINE_4+5+3, 0, &pMan[0],PAGE_HIGHLIGHT_COLOR, 0); 
        }
        ...
        if (currentItem == 0)
        {
          ...
          LCDD_ClearRect(PAGE_SENSITIVITY_NAME_X_POS, rect[currentItem].yStart, LCD_X_LENGTH-1, rect[currentItem].yEnd, 0);
          LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, yOffset, 0, DALI_zone_name[language], 0, 0);
       ...
       else 
        {
           ...
           LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, 18, 0, DALI_zone_name[language], 0, 0);
        }
 ...
 //PString=strings[0];
        ...
        _param_pageNOback_funckey_initializer(FUNCKEY_WRITE_ONLY);
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        ...
        {
          if(currentItem==0&& currentItem_x != 0xff )            
          {
            if(currentItem_x==0x01)
            {//exclude
              IR_ReadNBofControlGearParameter(NITOO_LM058A_NODE,0xB0,0x3C,0xff,0xff);
            }
...
#define PAGE_SENSITIVITY_BODY_Y_POS_3       (PAGE_SENSITIVITY_BODY_Y_POS_2 + PAGE_SENSITIVITY_BODY_Y_POS_DELTA)
  ...
  PAGE_PARAMETER *pTempParameter;
  yOffset = PAGE_SENSITIVITY_BODY_Y_POS_1;
  const unsigned char *ErrorStrings[]={"    ",DALI_wb_name[language],DALI_cb_name[language],DALI_wcb_name[language],DALI_wc_name[language]}; 
  const unsigned char *strings[]={DALI_windows_name[language],DALI_corridor_name[language],DALI_board_name[language],DALI_exclude_name[language]}; 
  static unsigned char currentItem_x  = 0xff;
  static const unsigned char *PString;
  unsigned char pMin[3];
  unsigned char pMan[3];
    if (pageInitialize)
    {
...
//          pageUpdate = 1;
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
        {
          ...
          if(currentItem==0 && currentItem_x != 0xff )//(strcmp((PString),ErrorStrings[0])!=0))// (zone_value)!=0 )           
          {
...
#define PAGE_VIEW_CURSORFLASH        1000*15
    static unsigned char invertCursor,invertCursor2,invertCursor3;
    static PAGE_VIEW_ITEM  view_item={0,0,0,0,0,0}; 
    static unsigned int timeStamp;
    static unsigned int timeStamp15S;
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_viewAssociation_name[language] ), 0, 0,DALI_viewAssociation_name[language] , 0, 0);
        ...
        //if (pageAvailability)
        {
          ...
          //if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_PIR_SENS))
          {
            ...
            LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, yOffset, 0, DALI_corridor_name[language], 0, 0);
            yOffset += PAGE_SENSITIVITY_BODY_Y_POS_DELTA;
          }
    ...
    if (pageUpdate)
    {
        ...
        for (i=0; i<totalItems; i++)
        {
            LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4+10, yOffset, LCD_X_LENGTH-2, yOffset+15, 0);
          //  LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS, yOffset, 0, strings[STRINGS_INDEX_LOW+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, (pParameter+i)->ParamIndex)+language], 0, 0);      // 2.4.0
            LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, yOffset, 0, "<>", 0, 0);
            yOffset += PAGE_SENSITIVITY_BODY_Y_POS_DELTA;
        }
        ...
        {
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_1+2, 0,DALI_on_name[language], 0, 0); 
        }
        else if(view.windows_value==0x66)
        {
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_1+2, 0,DALI_off_name[language], 0, 0);         
        }
        else if(view.windows_value==0xFF)
        {
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_1+2, 0,"--", 0, 0);          
        }
        else{
          dectoascii(&pDaylightsetpoint[0],view.windows_value, 3);
          strcpy((char *)&pDaylightsetpoint[3], "%");
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_1+2, 0, &pDaylightsetpoint[0], 0, 0);  
        }
           
        LCDD_PutString(20-5, PAGE_SETTINGS_LINE_4, 0, DALI_ballasts_name[language], 0, 0);
        dectoascii(&pDaylightsetpoint[0],view.win_ballast, 2);
        LCDD_PutString(5, PAGE_SETTINGS_LINE_4, 0, &pDaylightsetpoint[0],0, 0);
        _param_page_funckey_initializer(FUNCKEY_READ_ONLY);
        if(view_item.win_blink)
        {            
          LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4+10, PAGE_SENSITIVITY_BODY_Y_POS_1, LCD_X_LENGTH-2-10-3, PAGE_SENSITIVITY_BODY_Y_POS_1+15, 0);
          if(view_item.win_value==0x65)
          {
            LCDD_PutString(100-30, PAGE_SETTINGS_LINE_1+2, 0,DALI_on_name[language],invertCursor==1? PAGE_HIGHLIGHT_COLOR:0, 0); 
          }
          else if(view_item.win_value==0x66)
          {
            LCDD_PutString(100-30, PAGE_SETTINGS_LINE_1+2, 0,DALI_off_name[language],invertCursor==1? PAGE_HIGHLIGHT_COLOR:0, 0);         
          }
          _param_page_funckey_initializer(FUNCKEY_WRITE_ONLY);     
        }
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_RIGHT)
        {
          ...
          if(currentItem==0)
          {
            if(view.windows_value==0x65)
            {
                if(view_item.win_value==0x65)
                {                  
                  view_item.win_value=0;
                  view_item.win_blink=0;
                  pageUpdate = 1;
                }
                else if(view_item.win_value==0x66)
                {
            ...
            else if(view.windows_value==0x66)
            {
                ...
                {                  
                  view_item.win_value=0x65;
                  view_item.win_blink=1;
                  timeStamp15S=TIME_GetTime();   

                } 
            }
            else
            {
              if(view_item.win_value==0x66)
              {  
                view_item.win_value=0;
                view_item.win_blink=0;
                pageUpdate = 1;
        ...
        {
          if (view_item.win_blink || view_item.corid_blink || view_item.board_blink)
          {
            
            if(currentItem==0 && view.windows_value!=0xFF)
            {
              if(view_item.win_value==0x65)
              {
                
                LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4+10,  PAGE_SENSITIVITY_BODY_Y_POS_1, LCD_X_LENGTH-2,  PAGE_SENSITIVITY_BODY_Y_POS_1+15, 0);                
                LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, PAGE_SENSITIVITY_BODY_Y_POS_1, 0, "<>", 0, 0); 
                LCDD_PutString(100-30, PAGE_SETTINGS_LINE_1+2, 0,DALI_on_name[language], 0, 0); 
                LCDD_Refresh();
...
//        BUTTON_Acknowledge();
    ...
    if (view_item.win_blink )
    {
      ...
      if(TIME_GetTime()-timeStamp15S>PAGE_VIEW_CURSORFLASH)
      {
        view_item.win_blink=0;
        view_item.win_value=0;
        pageUpdate = 1;
...
#define PAGE_VIEW_CURSORFLASH        1000*15
    ...
    if (pageInitialize)
    {
        ...
        //if (pageAvailability)
        {
          ...
          //if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_PIR_SENS))
          {
            ...
            LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, yOffset, 0,DALI_windows_name[language], 0, 0);
            yOffset += PAGE_SENSITIVITY_BODY_Y_POS_DELTA;
          }
          //if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_US_SENS))
          {
          ...
          // if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_HF_SENS))
          {
            ...
            LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, yOffset, 0, DALI_board_name[language], 0, 0);
          }
    ...
    if (pageUpdate)
    {
        ...
        {
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_2+2, 0,DALI_on_name[language], 0, 0); 
        }
        else if(view.corridor_value==0x66)
        {
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_2+2, 0,DALI_off_name[language], 0, 0);         
        }
        else if(view.corridor_value==0xFF)
        {
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_2+2, 0,"--", 0, 0);          
        }
        else{
          
          dectoascii(&pDaylightsetpoint[0],view.corridor_value, 3);
          strcpy((char *)&pDaylightsetpoint[3], "%");
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_2+2, 0, &pDaylightsetpoint[0], 0, 0);  
        }
        
        if(view.board_value==0x65)
        {
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_3+2, 0,DALI_on_name[language], 0, 0); 
        }
        else if(view.board_value==0x66)
        {
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_3+2, 0,DALI_off_name[language], 0, 0);         
        }
        else if(view.board_value==0xFF)
        {
          LCDD_PutString(100-30, PAGE_SETTINGS_LINE_3+2, 0,"--", 0, 0);          
        }
        else{
          dectoascii(&pDaylightsetpoint[0],view.board_value, 3);
          strcpy((char *)&pDaylightsetpoint[3], "%");
          LCDD_PutString(100-30,  PAGE_SETTINGS_LINE_3+2, 0, &pDaylightsetpoint[0], 0, 0);  
        }
        ...
        if(currentItem == 1)
        {
          dectoascii(&pDaylightsetpoint[0],view.cori_ballast, 2);
          LCDD_PutString(5, PAGE_SETTINGS_LINE_4, 0, &pDaylightsetpoint[0],0, 0);
          
        }
        if(currentItem ==2)
        {
          dectoascii(&pDaylightsetpoint[0],view.board_ballast, 2);
          LCDD_PutString(5, PAGE_SETTINGS_LINE_4, 0, &pDaylightsetpoint[0],0, 0);
        }
        _param_page_funckey_initializer(FUNCKEY_READ_ONLY);
        if(view_item.win_blink)
        ...
        if(view_item.corid_blink)
        {            
          LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4+10, PAGE_SENSITIVITY_BODY_Y_POS_2, LCD_X_LENGTH-2-10-3, PAGE_SENSITIVITY_BODY_Y_POS_2+15, 0);
          if(view_item.corid_value==0x65)
          {
            LCDD_PutString(100-30, PAGE_SETTINGS_LINE_2+2, 0,DALI_on_name[language],invertCursor==1? PAGE_HIGHLIGHT_COLOR:0, 0); 
          }
          else if(view_item.corid_value==0x66)
          {
            LCDD_PutString(100-30, PAGE_SETTINGS_LINE_2+2, 0,DALI_off_name[language],invertCursor==1? PAGE_HIGHLIGHT_COLOR:0, 0);         
          }
          _param_page_funckey_initializer(FUNCKEY_WRITE_ONLY);
        }
        
        if(view_item.board_blink)
        {            
          LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4+10, PAGE_SENSITIVITY_BODY_Y_POS_3, LCD_X_LENGTH-2-10-3, PAGE_SENSITIVITY_BODY_Y_POS_3+15, 0);
          if(view_item.board_value==0x65)
          {
            LCDD_PutString(100-30, PAGE_SETTINGS_LINE_3+2, 0,DALI_on_name[language],invertCursor==1? PAGE_HIGHLIGHT_COLOR:0, 0); 
          }
          else if(view_item.board_value==0x66)
          {
            LCDD_PutString(100-30, PAGE_SETTINGS_LINE_3+2, 0,DALI_off_name[language],invertCursor==1? PAGE_HIGHLIGHT_COLOR:0, 0);         
          }
          _param_page_funckey_initializer(FUNCKEY_WRITE_ONLY);
        }
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_UP)
        {
          if (pageAvailability)
          {
            ...
            view_item.win_blink=0;
            view_item.corid_blink=0;
            view_item.board_blink=0;
            view_item.board_value=0;
            view_item.corid_value=0;
            view_item.win_value=0;
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_RIGHT)
        {
          ...
          if(currentItem==1)
          {
            ...
            else if(view.corridor_value==0x66)
            {
                ...
                {                  
                  view_item.corid_value=0x65;
                  view_item.corid_blink=1;
                  timeStamp15S=TIME_GetTime();   
          ...
          if(currentItem==2)
          {
            ...
            else if(view.board_value==0x66)
            {
                ...
                {                  
                  view_item.board_value=0x65;
                  view_item.board_blink=1;
                  timeStamp15S=TIME_GetTime();   
        ...
        {
          if(view_item.win_blink ==0 && view_item.corid_blink==0 && view_item.board_blink ==0)
          {
            int i=1;
            int result_read;
            for(i=1;i<4;i++)
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
        {
          ...
          if (view_item.win_blink || view_item.corid_blink || view_item.board_blink)
          {
            ...
            if(currentItem==1 && view.corridor_value!=0xFF)
            {
              if(view_item.corid_value==0x65)
              {
                LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4+10,  PAGE_SENSITIVITY_BODY_Y_POS_1+ PAGE_SENSITIVITY_BODY_Y_POS_DELTA, LCD_X_LENGTH-2,  PAGE_SENSITIVITY_BODY_Y_POS_1+15+ PAGE_SENSITIVITY_BODY_Y_POS_DELTA, 0);                
                LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, PAGE_SENSITIVITY_BODY_Y_POS_1+ PAGE_SENSITIVITY_BODY_Y_POS_DELTA, 0, "<>", 0, 0); 
                LCDD_PutString(100-30, PAGE_SETTINGS_LINE_2+2, 0,DALI_on_name[language], 0, 0); 
                LCDD_Refresh();
            ...
            if(currentItem==2 && view.board_value!=0xFF)
            {
              if(view_item.board_value==0x65)
              {
                LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4+10,  PAGE_SENSITIVITY_BODY_Y_POS_1+ PAGE_SENSITIVITY_BODY_Y_POS_DELTA+ PAGE_SENSITIVITY_BODY_Y_POS_DELTA, LCD_X_LENGTH-2,  PAGE_SENSITIVITY_BODY_Y_POS_1+15+ PAGE_SENSITIVITY_BODY_Y_POS_DELTA+ PAGE_SENSITIVITY_BODY_Y_POS_DELTA, 0);                
                LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, PAGE_SENSITIVITY_BODY_Y_POS_1+ PAGE_SENSITIVITY_BODY_Y_POS_DELTA+ PAGE_SENSITIVITY_BODY_Y_POS_DELTA, 0, "<>", 0, 0); 
                LCDD_PutString(100-30, PAGE_SETTINGS_LINE_3+2, 0,DALI_on_name[language], 0, 0); 
                LCDD_Refresh();
                ...
                IR_Read_UNIT_DESCRIPTION_Info(0x03);
                if(view.board_value==0x65)
                {
                  _param_page_funckey_initializer(FUNCKEY_READ_ONLY);
                  view_item.board_value=0;
...
{   if (pageUpdate)
    {
        ...
        else if(view.board_value==0x66)
        {
         ...
         LCDD_PutString(20-5, PAGE_SETTINGS_LINE_4, 0, "    Control Gear", 0, 0);
      ...
      if(firstDisplay==1)
      {
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F2)
        {
          ...
          else
          {
              ...
              result_read=IR_Read_UNIT_DESCRIPTION_Info(currentItem+1);
              if( result_read==2)
              {
                if(currentItem==0)
                  IR_ReadNBofControlGearParameter(NITOO_LM058A_NODE,CODE_OPEN_LEARNING,0x41,0x01,0x00);
...
#define PAGE_SUBREADPARAMS_BODY_Y_START     (PAGE_SUBREADPARAMS_RECT_Y_START+PAGE_SUBREADPARAMS_BODY_INDENT)
  ...
  unsigned char const *string = NULL;
  unsigned char currentParameter;
  const unsigned char *string_zone[]={DALI_error3_name[language]/*0"ERROR 3 too many ballasts 64 max"*/,\
    DALI_error4_name[language]/*1"ERROR 4 Limit system reached"*/,\
      "error",\
    DALI_waitEND_name[language] /*   3"wait end of fast blinking"*/,\
    DALI_error6_name[language] /*    4 "ERROR 6 Faulty ballast"*/,\
    DALI_error103_name[language]       /* 5"ERROR 103 Command denied"*/,\
    DALI_error7_name[language]         /* 6"ERROR 7 Address conflict Resolve before continue"*/,\
    DALI_error8_name[language]        /* 7   "ERROR 8 Corrupted data Resolve before continue"*/,"error fe","not fe ff"};
    const unsigned char* string_fuck={"crc error"};
     while (retry < PAGE_PARAM_READ_RETRY)
    {
        if (pageForceRead || (result =IR_ReadCreateZoneInfo(tempLanguage)))//= IR_ReadCreateZoneInfo()))///
        //if(1)
        {
#if defined MANTIS4974
          ...
          result = 6;
            ...
            {
            case 1:
                string= string_zone[0];/*"ERROR 3 too many ballasts 64 max"*/
                break;
            case 2:
                string = string_zone[1];/*"ERROR 4 Limit system reached"*/
                break;
            case 3:
                string = string_zone[2];
                break;
            case 4:
                string = string_zone[3];/*   "wait end of fast blinking"*/
                break;
            case 5:
                string = string_zone[5];/* "ERROR 103 Command denied"*/
                break;
            case 6:
              string = string_zone[4];/*     "ERROR 6 Faulty ballast"*/
              break;
            case 7:
              string = string_zone[6];/* "ERROR 7 Address conflict Resolve before continue"*/
              break;
            case 8:
              string = string_zone[7];/* 7   "ERROR 8 Corrupted data Resolve before continue"*/
              break;
            case 9:
              string = string_zone[8];
              break;   
            case 11:
              string = string_zone[9];
              break;   
            case 12:
              string = string_fuck;
              break;            
    ...
    if (string != NULL)
    {
        ...
        else
        {
          ...
          LCDD_PutString(PAGE_SUBREADPARAMS_BODY_X_START, PAGE_SUBREADPARAMS_BODY_Y_START, PAGE_SUBREADPARAMS_BODY_X_LENGTH, string, 3, 0);
          LCDD_Refresh();     // Have to do this here.
          if(result==1 || result==2 || result==5 ||result==6 ||result==7 ||result==8)
            TIME_Wait(5000);
          else
            TIME_Wait(2000);
           // if(result!=2)
...
#define PAGE_SUBREADPARAMS_BODY_Y_START     (PAGE_SUBREADPARAMS_RECT_Y_START+PAGE_SUBREADPARAMS_BODY_INDENT)
    ...
    unsigned char const *string = NULL;
    unsigned char currentParameter;
    const unsigned char *string_zone[]={DALI_error3_name1[language]/*"ERROR 3 Too many Sensor 64 max"*/,DALI_error4_name[language]/*"ERROR 4 Limit system reached"*/,"error",DALI_waitEND_name[language]/*"Wait end of fast blinking"*/};
    const unsigned char* string_fuck[]={"CRC error"};
     while (retry < PAGE_PARAM_READ_RETRY)
    {
        if(pageForceRead || (result = IR_ReadDetectorInfo()))
        {
            ...
            case 10:
              ...
              break;
            case 11:
              pageCurrent = PAGE_INDEX_NOGROUP;
              break;               
            default:
              string = strings[STRINGS_INDEX_ERRORREADING +language];
              break;
...
#define PAGE_SUBREADPARAMS_BODY_Y_START     (PAGE_SUBREADPARAMS_RECT_Y_START+PAGE_SUBREADPARAMS_BODY_INDENT)
    ...
    unsigned char const *string = NULL;
    unsigned char currentParameter;
    const unsigned char *string_zone[]={"Set error","flag error",DALI_error5long_name[language],"not enough error",DALI_error103_name[language]};
    const unsigned char* string_fuck[]={"CRC error"};
     while (retry < PAGE_PARAM_READ_RETRY)
    {
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        ...
        else if ((BUTTON_GetButtonIndex() == BUTTON_INDEX_F2) || pageForceRead)
        {
          if (pageAvailability)
          {
            ...
            int result;
            result=IR_ReadSensorParameter(&sensorParams, PARAM_INDEX_OFFset);
            //result = 5; ///for test lky
            if(result==5)
            {
              WaringBlock(DALI_error5_name[language],DALI_error5NeedtwoZone_name[language]);
              TIME_Wait(2000);
...
#define PAGE_DAYLIGHTSETPOINT_EYE_HEIGHT    32      // 2.3.0
    ...
    if (pageInitialize)
    {
       ...
       LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, DALI_DetectionLED_name[language]), 0, 0, DALI_DetectionLED_name[language], 0, 0);
    ...
    if (pageUpdate)
    {
        // 2.3.0
         ...
         if (temp)
          strcpy((char *)&pDaylightsetpoint[0],DALI_on_name[language]);
         else
          strcpy((char *)&pDaylightsetpoint[0], DALI_off_name[language]);          
         LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, &pDaylightsetpoint[0]), PAGE_DAYLIGHTSETPOINT_BODY_Y_POS, 0, &pDaylightsetpoint[0], PAGE_HIGHLIGHT_COLOR, 0);    // 2.2.0
...
{
//#define PAGE_SENSITIVITY_TITLE_X_POS        36
#define PAGE_SENSITIVITY_MAXIMUMITEMS      2
#define PAGE_SENSITIVITY_NAME_X_POS         4
#define PAGE_SENSITIVITY_VALUE_X_POS        40+40+10
#define PAGE_SENSITIVITY_BRACKET_X_POS      114
#define PAGE_SENSITIVITY_BODY_Y_POS_1       18
#define PAGE_SENSITIVITY_BODY_Y_POS_DELTA   22
#define PAGE_SENSITIVITY_BODY_Y_POS_2       (PAGE_SENSITIVITY_BODY_Y_POS_1 + PAGE_SENSITIVITY_BODY_Y_POS_DELTA)
#define PAGE_SENSITIVITY_BODY_Y_POS_3       (PAGE_SENSITIVITY_BODY_Y_POS_2 + PAGE_SENSITIVITY_BODY_Y_POS_DELTA)
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_AUX_MODE) || PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_AUX_PULE) ;

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1,strings_aux_input_titile[language]), 0, 0, strings_aux_input_titile[language], 0, 0);
        ...
        if (pageAvailability)
        {
          ...
          pTempParameter = pParameter;
          yOffset = PAGE_SENSITIVITY_BODY_Y_POS_1;
          if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_AUX_MODE))
          {
            totalItems++;
            pTempParameter->ParamIndex = PARAM_INDEX_AUX_MODE;
            pTempParameter->Max = 1;
            pTempParameter->Min = 0;
            pTempParameter++;
            LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, yOffset, 0, strings_aux_input_mode_titile[language], 0, 0);
            yOffset += PAGE_SENSITIVITY_BODY_Y_POS_DELTA;
          }
          if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_AUX_PULE))
          {
            totalItems++;
            pTempParameter->ParamIndex = PARAM_INDEX_AUX_PULE;
            pTempParameter->Max = 50;
            pTempParameter->Min = 2;
            pTempParameter++;
            //LCDD_PutString(0, yOffset, 0, strings_aux_input_short_pulse_titile[language], 0, 0);
            yOffset += PAGE_SENSITIVITY_BODY_Y_POS_DELTA;
          }
          pageUpdate = 1;
        }
    ...
    if (pageUpdate)
    {
      ...
      if(LCDD_PutScrollString(0,40,90-5-1,(strings_aux_input_short_pulse_titile[language]+((1 == currentItem)?pageScroll:0)), (1 == currentItem)?PAGE_HIGHLIGHT_COLOR:0, (1 == currentItem)?1:0))
        
      {
       ...
       // for (i=0; i<totalItems; i++)
        {
          LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4-5-5, yOffset, LCD_X_LENGTH-2, yOffset+15, 0);
          // LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS, yOffset, 0, strings[STRINGS_INDEX_LOW+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, (pParameter+i)->ParamIndex)+language], 0, 0);      // 2.4.0
         // dectoascii(&pr[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DIM_R), 2);
          dectoascii(&pf[0], (PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_AUX_PULE))/10, 1);
          dectoascii(&pr[0], (PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_AUX_PULE))%10, 1);
          int i;
          i= PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_AUX_MODE);
          if(i)
            LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS-20-5-5-5, yOffset, 0,strings_button_titile[language], 0, 0);  
          else
            LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS-20-5-5-5, yOffset, 0,strings_switch_titile[language], 0, 0);  
         // LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS+13, yOffset, 0,strings_dim_sec_titile[language] , 0, 0);  
          LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, yOffset, 0, "<>", 0, 0);
          yOffset += PAGE_SENSITIVITY_BODY_Y_POS_DELTA;
          LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4-2, yOffset, LCD_X_LENGTH-2, yOffset+15, 0);  
          //LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS-10+2, yOffset, 0,"0.", 0, 0); 
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS-10+2-3+2, yOffset, 0," ", 0, 0);
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS-10+2, yOffset, 0,&pf[0], 0, 0);
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS-10+2+5, yOffset, 0,".", 0, 0);
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS+2, yOffset, 0,&pr[0] , 0, 0);  
          LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, yOffset, 0, "<>", 0, 0);
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS+13, yOffset, 0,strings_dim_sec_titile[language] , 0, 0); 
        }
...
//            pageUpdate = 1;
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
        {
            if (pageAvailability)
            {
            _page_sub_set_parameters((pParameter+currentItem), 1);
...
#define PAGE_SENSITIVITY_MAXIMUMITEMS      2
#define PAGE_SENSITIVITY_NAME_X_POS         4
#define PAGE_SENSITIVITY_VALUE_X_POS        40+40
#define PAGE_SENSITIVITY_BRACKET_X_POS      114
#define PAGE_SENSITIVITY_BODY_Y_POS_1       18
#define PAGE_SENSITIVITY_BODY_Y_POS_DELTA   22
#define PAGE_SENSITIVITY_BODY_Y_POS_2       (PAGE_SENSITIVITY_BODY_Y_POS_1 + PAGE_SENSITIVITY_BODY_Y_POS_DELTA)
#define PAGE_SENSITIVITY_BODY_Y_POS_3       (PAGE_SENSITIVITY_BODY_Y_POS_2 + PAGE_SENSITIVITY_BODY_Y_POS_DELTA)
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DIM_F) || PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DIM_R) ;

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1,strings_dim_slop_titile[language]), 0, 0, strings_dim_slop_titile[language], 0, 0);
        ...
        if (pageAvailability)
        {
          ...
          pTempParameter = pParameter;
          yOffset = PAGE_SENSITIVITY_BODY_Y_POS_1;
          if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DIM_R))
          {
            totalItems++;
            pTempParameter->ParamIndex = PARAM_INDEX_DIM_R;
            pTempParameter->Max = 30;
              pTempParameter->Min = 1;
            pTempParameter++;
            LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, yOffset, 0, strings_dim_rise_titile[language], 0, 0);
            yOffset += PAGE_SENSITIVITY_BODY_Y_POS_DELTA;
          }
          if (PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_DIM_F))
          {
            totalItems++;
            pTempParameter->ParamIndex = PARAM_INDEX_DIM_F;
            pTempParameter->Max = 30;
              pTempParameter->Min = 1;
            pTempParameter++;
            LCDD_PutString(PAGE_SENSITIVITY_NAME_X_POS, yOffset, 0, strings_dim_fall_titile[language], 0, 0);
            yOffset += PAGE_SENSITIVITY_BODY_Y_POS_DELTA;
          }
          pageUpdate = 1;
        }
       ...
       // for (i=0; i<totalItems; i++)
        {
          LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4, yOffset, LCD_X_LENGTH-2, yOffset+15, 0);
          // LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS, yOffset, 0, strings[STRINGS_INDEX_LOW+STRINGS_TOTAL_LANGUAGES*PARAMS_GetParameterValue(&sensorParams, (pParameter+i)->ParamIndex)+language], 0, 0);      // 2.4.0
          if(PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DIM_R)>=10)
            dectoascii(&pr[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DIM_R), 2);
          if(PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DIM_R)<10)  
            dectoascii(&pr[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DIM_R), 1);
          if(PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DIM_F)>=10)
            dectoascii(&pf[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DIM_F), 2);
          if(PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DIM_F)<10)             
            dectoascii(&pf[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_DIM_F), 1);
          
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS, yOffset, 0,&pr[0] , 0, 0);  
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS+13, yOffset, 0,strings_dim_sec_titile[language] , 0, 0);  
          LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, yOffset, 0, "<>", 0, 0);
          yOffset += PAGE_SENSITIVITY_BODY_Y_POS_DELTA;
          LCDD_ClearRect(PAGE_SENSITIVITY_VALUE_X_POS-4, yOffset, LCD_X_LENGTH-2, yOffset+15, 0);  
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS, yOffset, 0,&pf[0] , 0, 0);  
          LCDD_PutString(PAGE_SENSITIVITY_BRACKET_X_POS, yOffset, 0, "<>", 0, 0);
          LCDD_PutString(PAGE_SENSITIVITY_VALUE_X_POS+13, yOffset, 0,strings_dim_sec_titile[language] , 0, 0); 
        }
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        ...
        else if ((BUTTON_GetButtonIndex() == BUTTON_INDEX_F2) || pageForceRead)
        {
            ...
            {
              // Temporarily reset to read all sensitivity parameters
             // pParameter->ParamIndex = PARAM_NUMBER_DIM_R;
             // (pParameter+1)->ParamIndex = PARAM_NUMBER_DIM_F;
              _page_sub_read_parameters(pParameter, 2);
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_F3)
        {
            if (pageAvailability)
            {
            _page_sub_set_parameters(pParameter, 2);
...
#define PAGE_STANDBYLEVEL_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_ON_LEVEL);

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings_on_level_titile[language]), 0, 0, strings_on_level_titile[language], 0, 0);
        ...
        if (pageAvailability)
        {
            ...
            pParameter->ParamIndex = PARAM_INDEX_ON_LEVEL;
            pParameter->Max = sizeof(valuesStandbyLevel)/sizeof(unsigned short)-1;
            //pParameter->Min = 0;
            if(sensorParams.Type.Value==0x107 || sensorParams.Type.Value==0x111 || sensorParams.Type.Value==0x112)
              pParameter->Min = 4;
    ...
    if (pageUpdate)
    {
        ...
        memset(&pStandbylevel[0], 0, sizeof(pStandbylevel));
        i = dectoascii(&pStandbylevel[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_ON_LEVEL), 0);
        pStandbylevel[i++] = ' ';
        pStandbylevel[i] = '%';
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, &pStandbylevel[0]), PAGE_STANDBYLEVEL_BODY_Y_POS, 0, &pStandbylevel[0], PAGE_HIGHLIGHT_COLOR, 0);
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_UP)
        {
            if (pageAvailability)
            {
                if (valuesStandbyLevel[pParameter->Max] <= PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_ON_LEVEL))
                {
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_ON_LEVEL, valuesStandbyLevel[pParameter->Min]);
                }
                else
                {
                    for (i=pParameter->Min; i<=pParameter->Max; i++)
                    {
                        if (valuesStandbyLevel[i] > PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_ON_LEVEL))
                        {
                            break;
                        }
                    }
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_ON_LEVEL, valuesStandbyLevel[i]);
                }
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_DOWN)
        {
            if (pageAvailability)
            {
                if (valuesStandbyLevel[pParameter->Min] >= PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_ON_LEVEL))
                {
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_ON_LEVEL, valuesStandbyLevel[pParameter->Max]);
                }
                else
                {
                    for (i=pParameter->Max; i>=pParameter->Min; i--)
...
void ExtendModePageHandler()
{
#define PAGE_PARAMETERS_MAXIMUMITEMS    5      // 1.0.24, was 6
#define PAGE_PARAMETERS_LINE_1          16
#define PAGE_PARAMETERS_LINE_HEIGHT     16
#define PAGE_PARAMETERS_LINE_SPACE      6
#define PAGE_PARAMETERS_LINE_2          (PAGE_PARAMETERS_LINE_1 + PAGE_PARAMETERS_LINE_HEIGHT + PAGE_PARAMETERS_LINE_SPACE)
...
#define PAGE_PARAMETERS_LINE_4          (PAGE_PARAMETERS_LINE_3 + PAGE_PARAMETERS_LINE_HEIGHT + PAGE_PARAMETERS_LINE_SPACE)
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(PAGE_LINE_INDENT+15+5, 0, 0, string_extended_mode_title[language], 0, 0);
        ...
        // 2.4.0
        {
            pTempParams->pString = string_extended_state_title[language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_EXTENDMODESATE;
            pTempParams++;
        }
        
        {
            ...
            pTempParams->pString = strings[STRINGS_INDEX_MODE+language];            
            pTempParams->PageHandlerIndex = PAGE_INDEX_EXTENDFUNC;
            pTempParams++;
        }
        
        {
            pTempParams->pString = strings[STRINGS_INDEX_STANDBYLEVEL+language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_EXTENDSTANDBY;
            pTempParams++;
        }

        {
            ...
            pTempParams->pString = strings[STRINGS_INDEX_STANDBYDELAY+language];            
            pTempParams->PageHandlerIndex = PAGE_INDEX_EXTENDDELAY;
            pTempParams++;
        }
        
        {
            pTempParams->pString = strings_on_level_titile[language];
            pTempParams->PageHandlerIndex = PAGE_INDEX_EXTENDONLEVEL;
            pTempParams++;
        }
...
void ExtendModeStatePageHandler()
{
  //#define PAGE_MODE_TITLE_X_POS       48
#define PAGE_MODE_NAME_X_POS        2
#define PAGE_MODE_VALUE_X_POS       112
#define PAGE_MODE_BODY_Y_POS_1      18+16
#define PAGE_MODE_BODY_Y_POS_2      40+16
#define PAGE_MODE_BODY_Y_POS_3      62
#define PAGE_MODE_BODY_Y_POS_4      84
...
//    const unsigned char *string;      // 2.3.0
    ...
    if (pageInitialize)
    {
        ...
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1,string_extended_mode_state_title[language]), 0, 0, string_extended_mode_state_title[language], 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1,string_extended_mode_state1_title[language]), 14, 0, string_extended_mode_state1_title[language], 0, 0);
        ...
        if (pageAvailability)
        {
          ...
          pParameter = (PAGE_PARAMETER *)malloc(2*sizeof(PAGE_PARAMETER));        // 2.3.1, was 1
          pParameter->ParamIndex = PARAM_INDEX_EXT_MODE_STATE;
          mode_value = PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_MODE_STATE);
    ...
    if (pageUpdate)
    {
        ...
        for (i=firstDisplayedItem; i<firstDisplayedItem+4; i++)
        {
            if (i < totalItems)
            {
                LCDD_ClearRect(0, rect[i-firstDisplayedItem].yStart, LCD_X_LENGTH-1, rect[i-firstDisplayedItem].yEnd, (i == currentItem) ? PAGE_HIGHLIGHT_COLOR:0);
                // 2.4.0
               LCDD_PutString(PAGE_MODE_NAME_X_POS, rect[i-firstDisplayedItem].yStart+1, 0, strings_extend_mode_state_titile[language+i*LANGUAGES_TOTAL], (i == currentItem) ? PAGE_HIGHLIGHT_COLOR:0, 0);
//                if (LCDD_PutScrollString(PAGE_MODE_NAME_X_POS, rect[i-firstDisplayedItem].yStart+1, (PAGE_MODE_VALUE_X_POS-2-PAGE_MODE_NAME_X_POS), strings[STRINGS_INDEX_MODE+possible_values[i]*STRINGS_TOTAL_LANGUAGES+language]+((i == currentItem)?pageScroll:0), (i == currentItem)?PAGE_HIGHLIGHT_COLOR:0, (i == currentItem)?1:0))
//                {
//                    pageScroll++;
//                    pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
//                }
                LCDD_ClearRect(PAGE_MODE_VALUE_X_POS-2, rect[i-firstDisplayedItem].yStart+1, LCD_X_LENGTH-2, rect[i-firstDisplayedItem].yEnd-1, 0);
                LCDD_PutBitmap(PAGE_MODE_VALUE_X_POS, rect[i-firstDisplayedItem].yStart+1, (mode_value == i)?&BitmapInfo_Validation[1]:&BitmapInfo_Validation[0], 0);
            }
        }
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_MAJOR)
        {
            if (pageAvailability)
            {
            mode_value = currentItem;
            PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_MODE_STATE, mode_value);
...
//            pageUpdate = 1;
        ...
        {
            pageCurrent = PAGE_INDEX_EXTENDMODE;//PAGE_INDEX_PARAMETERS;
...
void ExtendModeFunctionPageHandler()
{
#define PAGE_MODE_TITLE_X_POS       48
#define PAGE_MODE_NAME_X_POS        2
#define PAGE_MODE_VALUE_X_POS       112
...
//    const unsigned char *string;      // 2.3.0
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_FUN_MODE);

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, string_extended_mode_function_title[language]), 0, 0, string_extended_mode_function_title[language], 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, string_extended_mode_function1_title[language]), 14, 0, string_extended_mode_function1_title[language], 0, 0);

        if (pageAvailability)
        {
        _param_page_funckey_initializer(FUNCKEY_READ_WRITE);

        pParameter = (PAGE_PARAMETER *)malloc(sizeof(PAGE_PARAMETER));        // 2.3.1, was 1
        pParameter->ParamIndex = PARAM_INDEX_EXT_FUN_MODE;
        mode_value = PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_FUN_MODE);
    ...
    if (pageUpdate)
    {
        ...
        for (i=firstDisplayedItem; i<firstDisplayedItem+4; i++)
        {
            if (i < totalItems)
            {
                LCDD_ClearRect(0, rect[i-firstDisplayedItem].yStart, LCD_X_LENGTH-1, rect[i-firstDisplayedItem].yEnd, (i == currentItem) ? PAGE_HIGHLIGHT_COLOR:0);
                // 2.4.0
//                LCDD_PutString(PAGE_MODE_NAME_X_POS, rect[i-firstDisplayedItem].yStart+1, 0, strings[STRINGS_INDEX_MODE+possible_values[i]*STRINGS_TOTAL_LANGUAGES+language], (i == currentItem) ? PAGE_HIGHLIGHT_COLOR:0, 0);
                if (LCDD_PutScrollString(PAGE_MODE_NAME_X_POS, rect[i-firstDisplayedItem].yStart+1, (PAGE_MODE_VALUE_X_POS-2-PAGE_MODE_NAME_X_POS), strings[STRINGS_INDEX_MODE+possible_values[i]*STRINGS_TOTAL_LANGUAGES+language]+((i == currentItem)?pageScroll:0), (i == currentItem)?PAGE_HIGHLIGHT_COLOR:0, (i == currentItem)?1:0))
                {
                    pageScroll++;
                    pageTimeStamp = TIME_GetTime() + LCDD_SCROLL_DELAY;
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_MAJOR)
        {
            ...
            {
            mode_value = possible_values[currentItem];
            PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_FUN_MODE, mode_value);
...
void ExtendStandbyPageHandler()
{
  #define PAGE_STANDBYLEVEL_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL);

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, string_extended_mode_standbyLevel_title[language]), 0, 0, string_extended_mode_standbyLevel_title[language], 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, string_extended_mode_standbyLevel1_title[language]), 14, 0, string_extended_mode_standbyLevel1_title[language], 0, 0);
        ...
        if (pageAvailability)
        {
            ...
            pParameter->ParamIndex = PARAM_INDEX_EXT_STANDBY_LEVEL;
            pParameter->Max = sizeof(valuesStandbyLevel)/sizeof(unsigned short)-1;
            pParameter->Min = 0;
    ...
    if (pageUpdate)
    {
        ...
        memset(&pStandbylevel[0], 0, sizeof(pStandbylevel));
        i = dectoascii(&pStandbylevel[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL), 0);
        pStandbylevel[i++] = ' ';
        pStandbylevel[i] = '%';
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, &pStandbylevel[0]), PAGE_STANDBYLEVEL_BODY_Y_POS, 0, &pStandbylevel[0], PAGE_HIGHLIGHT_COLOR, 0);
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_UP)
        {
            if (pageAvailability)
            {
                if (valuesStandbyLevel[pParameter->Max] <= PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL))
                {
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL, valuesStandbyLevel[pParameter->Min]);
                }
                else
                {
                    for (i=pParameter->Min; i<=pParameter->Max; i++)
                    {
                        if (valuesStandbyLevel[i] > PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL))
                        {
                            break;
                        }
                    }
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL, valuesStandbyLevel[i]);
                }
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_DOWN)
        {
            if (pageAvailability)
            {
                if (valuesStandbyLevel[pParameter->Min] >= PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL))
                {
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_LEVEL, valuesStandbyLevel[pParameter->Max]);
                }
                else
                {
                    for (i=pParameter->Max; i>=pParameter->Min; i--)
...
#define PAGE_STANDBYDELAY_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_STANDBY_DELAY);

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, string_extended_mode_standbyDelay_title[language]), 0, 0, string_extended_mode_standbyDelay_title[language], 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, string_extended_mode_standbyDelay1_title[language]), 14, 0, string_extended_mode_standbyDelay1_title[language], 0, 0);
        ...
        if (pageAvailability)
        {
            ...
            pParameter->ParamIndex = PARAM_INDEX_EXT_STANDBY_DELAY;
            pParameter->Max = sizeof(valuesStandbyDelay)/sizeof(unsigned short)-1;
            pParameter->Min = 0;
    ...
    if (pageUpdate)
    {
        ...
        memset(&pStandbydelay[0], 0, sizeof(pStandbydelay));
        tempvalue = PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_DELAY);
        if (tempvalue == 0)
        {
            strcpy((char *)&pStandbydelay[0], (char const *)strings[STRINGS_INDEX_NOSTANDBY+language]);
        }
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_UP)
        {
            if (pageAvailability)
            {
                if (valuesStandbyDelay[pParameter->Max] <= PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_DELAY))
                {
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_DELAY, valuesStandbyDelay[pParameter->Min]);
                }
                else
                {
                    for (i=pParameter->Min; i<=pParameter->Max; i++)
                    {
                        if (valuesStandbyDelay[i] > PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_DELAY))
                        {
                            break;
                        }
                    }
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_DELAY, valuesStandbyDelay[i]);
                }
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_DOWN)
        {
            if (pageAvailability)
            {
                if (valuesStandbyDelay[pParameter->Min] >= PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_DELAY))
                {
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_STANDBY_DELAY, valuesStandbyDelay[pParameter->Max]);
                }
                else
                {
                    for (i=pParameter->Max; i>=pParameter->Min; i--)
...
#define PAGE_STANDBYLEVEL_BODY_Y_POS    PAGE_VALUE_Y_POS
    ...
    if (pageInitialize)
    {
        ...
        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_EXT_ON_LEVEL);

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings_extended_on_level_titile[language]), 0, 0, strings_extended_on_level_titile[language], 0, 0);
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings_extended_on_level1_titile[language]), 14, 0, strings_extended_on_level1_titile[language], 0, 0);
        ...
        if (pageAvailability)
        {
            ...
            pParameter->ParamIndex = PARAM_INDEX_EXT_ON_LEVEL;
            pParameter->Max = sizeof(valuesStandbyLevel)/sizeof(unsigned short)-1;
            
            if(sensorParams.Type.Value==0x107 || sensorParams.Type.Value==0x111)
              pParameter->Min = 4;
            else
              pParameter->Min = 0;
    ...
    if (pageUpdate)
    {
        ...
        memset(&pStandbylevel[0], 0, sizeof(pStandbylevel));
        i = dectoascii(&pStandbylevel[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_ON_LEVEL), 0);
        pStandbylevel[i++] = ' ';
        pStandbylevel[i] = '%';
        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, &pStandbylevel[0]), PAGE_STANDBYLEVEL_BODY_Y_POS, 0, &pStandbylevel[0], PAGE_HIGHLIGHT_COLOR, 0);
    ...
    if ((BUTTON_GetMessage() == BUTTON_MESSAGE_PUSHED) || (BUTTON_GetMessage() == BUTTON_MESSAGE_RELEASED) || pageForceRead)
    {
        if (BUTTON_GetButtonIndex() == BUTTON_INDEX_UP)
        {
            if (pageAvailability)
            {
                if (valuesStandbyLevel[pParameter->Max] <= PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_ON_LEVEL))
                {
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_ON_LEVEL, valuesStandbyLevel[pParameter->Min]);
                }
                else
                {
                    for (i=pParameter->Min; i<=pParameter->Max; i++)
                    {
                        if (valuesStandbyLevel[i] > PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_ON_LEVEL))
                        {
                            break;
                        }
                    }
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_ON_LEVEL, valuesStandbyLevel[i]);
                }
        ...
        else if (BUTTON_GetButtonIndex() == BUTTON_INDEX_DOWN)
        {
            if (pageAvailability)
            {
                if (valuesStandbyLevel[pParameter->Min] >= PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_EXT_ON_LEVEL))
                {
                    PARAMS_SetParameterValue(&sensorParams, PARAM_INDEX_EXT_ON_LEVEL, valuesStandbyLevel[pParameter->Max]);
                }
                else
                {
                    for (i=pParameter->Max; i>=pParameter->Min; i--)
...
void OperationTimeOnPageHandler()
{
  #define PAGE_DAYLIGHTLEVEL_BODY_X_POS         40
#define PAGE_DAYLIGHTLEVEL_BODY_Y_POS         PAGE_VALUE_Y_POS

    static PAGE_PARAMETER *pParameter;
    unsigned char pDaylightlevel[6];   // 2.2.0, was 6

    if (pageInitialize)
    {
        pageInitialize = 0;

        pageAvailability = PARAMS_IsParameterAvailable(&sensorParams, PARAM_INDEX_OP_TIME);

        LCDD_ClearRect(0, 0, LCD_X_LENGTH-1, LCD_Y_LENGTH-1, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings_operating_time_titile[language]), 0, 0, strings_operating_time_titile[language], 0, 0);

        LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, strings_operating_time1_titile[language]), 20, 0, strings_operating_time1_titile[language], 0, 0);
        ...
        pParameter->ParamIndex = PARAM_INDEX_OP_TIME;
       // pParameter->Max = 0x65535;
       // pParameter->Min = 0;
    ...
    if (pageUpdate)
    {
        if (pageAvailability)
        {  
            dectoascii(&pDaylightlevel[0], PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_OP_TIME), get_digits(PARAMS_GetParameterValue(&sensorParams, PARAM_INDEX_OP_TIME))); 
            //strcpy((char *)&pDaylightlevel[temp], strings_operating_time_hours_titile[language]);
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, &pDaylightlevel[0])-10, PAGE_DAYLIGHTLEVEL_BODY_Y_POS, 0, &pDaylightlevel[0], PAGE_HIGHLIGHT_COLOR, 0);     // 2.2.0
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, &pDaylightlevel[0])-10+38, PAGE_DAYLIGHTLEVEL_BODY_Y_POS, 0, strings_hours_titile[language], PAGE_HIGHLIGHT_COLOR, 0);
        }
        else
        {
            LCDD_PutString(_figure_string_center_x_pos(0, LCD_X_LENGTH-1, " N/A "), PAGE_VALUE_Y_POS, 0, " N/A ", 0, 0);
